...
public static class JuniperStoreRegistry {
    ...
    public static void Initialize(IEnumerable<JuniperStore> stores) {
        lock (Lock) {
            ByProductId.Clear();
            ByStoreId.Clear();
            foreach (JuniperStore store in stores)
                RegisterCore(store);
        }
    }
    ...
    private static void RegisterCore(JuniperStore store) {
        ...
        foreach (JuniperProduct product in store.Products)
    ...
    public static void RegisterProduct(JuniperProduct product) {
        lock (Lock) {
            ByProductId[product.ProductId] = product;
            if (ByStoreId.TryGetValue(product.StoreId, out JuniperStore? store) &&
    ...
    public static bool TryGetProduct(BigInteger productId, [NotNullWhen(true)] out JuniperProduct? product) {
        lock (Lock) {
            return ByProductId.TryGetValue(productId, out product);
        }
    }

    public static bool TryGetProduct(string? productId, [NotNullWhen(true)] out JuniperProduct? product) {
        product = null;
        return TryParseProductId(productId, out BigInteger id) && TryGetProduct(id, out product);
    }
    ...
    public static bool TryGetStore(Guid rowId, [NotNullWhen(true)] out JuniperStore? store) {
        lock (Lock) {
            return ByStoreId.TryGetValue(rowId, out store);
        }
    }

    public static string GetStoreName(Guid rowId) {
        return TryGetStore(rowId, out JuniperStore? store) ? store.StoreName : string.Empty;
    }

    public static IReadOnlyList<JuniperStore> AllStores() {
        lock (Lock) {
            return ByStoreId.Values.OrderBy(s => s.StoreName, StringComparer.OrdinalIgnoreCase).ToList();
        }
    }

    public static IReadOnlyList<JuniperProduct> AllProducts() {
        lock (Lock) {
            return ByProductId.Values.OrderBy(p => p.ProductName, StringComparer.OrdinalIgnoreCase).ToList();
        }
    }

    public static void RemoveProduct(BigInteger productId) {
        lock (Lock) {
            if (!ByProductId.Remove(productId, out JuniperProduct? product)) return;
            if (ByStoreId.TryGetValue(product.StoreId, out JuniperStore? store))
    ...
    public static void RemoveStore(Guid rowId) {
        lock (Lock) {
            if (!ByStoreId.Remove(rowId, out JuniperStore? store)) return;
            foreach (JuniperProduct product in store.Products)
    ...
    public static JuniperStore GetOrProvisionStore(string storeName) {
        JuniperStore store;
        lock (Lock) {
            JuniperStore? existing = ByStoreId.Values.FirstOrDefault(s =>
    ...
    public static void RaiseStoreUpdated(JuniperStore store) {
        StoreUpdated?.Invoke(store);
    }

    public static void RaiseProductUpdated(JuniperProduct product) {
...
public static class JuniperOrderHelper {
    ...
    public static string GetProductLabel(string? meta) {
        if (!TryGetProduct(meta, out JuniperProduct? product))
    ...
    public static string GetStoreLabel(string? meta) {
        if (TryGetProduct(meta, out JuniperProduct? product)) {
            string name = JuniperStoreRegistry.GetStoreName(product.StoreId);
...
public static class OrderMetaFilter {
    public static bool Matches(SubathonEventType? eventType, string? eventMeta, string? filter) {
        ...
        if (eventType == SubathonEventType.JuniperMerchSale && Guid.TryParse(filter, out Guid storeId)
                                                            && JuniperOrderHelper.TryGetProduct(eventMeta,
                                                                out JuniperProduct? product))
    ...
    public static string Describe(SubathonEventType? eventType, string? filter) {
        ...
        if (eventType == SubathonEventType.JuniperMerchSale) {
            if (Guid.TryParse(filter, out Guid storeId)) {
                string storeName = JuniperStoreRegistry.TryGetStore(storeId, out JuniperStore? store)
                    ? store.StoreName
                    : "Unknown Store";
...
public static class GoAffProStoreRegistry {
    ...
    public static void Initialize(IEnumerable<GoAffProStore> stores) {
        lock (Lock) {
            ...
            foreach (GoAffProStore store in stores)
                Register(store);
        }
    }
    ...
    public static bool TryGetBySiteId(int siteId, [NotNullWhen(true)] out GoAffProStore? store) {
        lock (Lock) {
            return BySiteId.TryGetValue(siteId, out store);
        }
    }

    public static bool TryGetByInternalName(string name, [NotNullWhen(true)] out GoAffProStore? store) {
        lock (Lock) {
            return ByInternalName.TryGetValue(name, out store);
        }
    }

    public static IReadOnlyList<GoAffProStore> All() {
        lock (Lock) {
            return BySiteId.Values.OrderBy(s => s.RowId).ToList();
        }
    }
    ...
    public static bool IsActiveOnAccount(int siteId) {
        lock (Lock) {
            return ActiveSiteIds.Contains(siteId);
        }
    }

    public static GoAffProStore GetOrProvision(int siteId, string fallbackName = "") {
        lock (Lock) {
            if (BySiteId.TryGetValue(siteId, out GoAffProStore? existing))
                return existing;
...
public static class GoAffProOrderHelper {
    ...
    public static bool TryGetStore(string? meta, [NotNullWhen(true)] out GoAffProStore? store) {
        store = null;
        return TryParseMeta(meta, out int siteId) && GoAffProStoreRegistry.TryGetBySiteId(siteId, out store);
    }

    public static string GetOrderLabel(string? meta) {
        if (!TryGetStore(meta, out GoAffProStore? store))
