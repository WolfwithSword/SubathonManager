...
public class EventTypeMetaAttribute : EnumMetaAttribute {
    public override string? Description => Source is not (SubathonEventSource.Command or SubathonEventSource.Unknown)
        ? $"{Source.ToString()} {Label}".Trim()
        : Label;
...
public static class EnumMetaCache {
    ...
    public static T? Get<T>(Enum value) where T : EnumMetaAttribute {
        ...
        Dictionary<object, EnumMetaAttribute?> map = Cache.GetOrAdd(type, t => {
        ...
        if (!map.TryGetValue(value, out EnumMetaAttribute? attr))
            return null;
