...
public static class AutoCompleteBoxBehaviours {
    ...
    public static void SetOpenOnFocus(AutoCompleteBox box, bool value) {
        box.SetValue(OpenOnFocusProperty, value);
    }

    public static bool GetOpenOnFocus(AutoCompleteBox box) {
        return box.GetValue(OpenOnFocusProperty);
    }

    private static void OnTemplateApplied(object? sender, TemplateAppliedEventArgs e) {
        if (sender is not AutoCompleteBox box) return;

        IBrush brush = box.TryFindResource("TextFillColorSecondaryBrush", box.ActualThemeVariant, out object? b) &&
                       b is IBrush ib
            ? ib
            : Brushes.Gray;
