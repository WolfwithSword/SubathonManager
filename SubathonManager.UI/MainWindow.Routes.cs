...
public partial class MainWindow {
    ...
    private void LoadRoutes() {
        ...
        List<Route> routes = db.Routes.OrderByDescending(r => r.UpdatedTimestamp).ToList();
    ...
    private async void CopyRouteUrl_Click(object? sender, RoutedEventArgs e) {
        try {
            if (sender is Control { DataContext: Route route }) {
                ...
                await UiHelpers.TrySetClipboardTextAsync(route.GetRouteUrl(config));
            }
        }
        catch (Exception ex) {
    ...
    private void DeleteRoute_Click(object? sender, RoutedEventArgs e) {
        if (sender is Control { DataContext: Route route }) {
            using AppDbContext db = _factory.CreateDbContext();
            Route? found = db.Routes.FirstOrDefault(r => r.Id == route.Id);
            if (found != null) {
    ...
    private void DuplicateRoute_Click(object? sender, RoutedEventArgs e) {
        if (sender is Control { DataContext: Route route }) {
            using AppDbContext db = _factory.CreateDbContext();
            Route? dbRoute = db.Routes.Include(r => r.Widgets)
            ...
            foreach (Widget widget in dbRoute.Widgets.ToArray()) {
                Widget cloneWidget = widget.Clone(clone.Id, widget.Name, widget.Z);
    ...
    private async Task<bool> ImportRouteFromFile(string filePath) {
        ...
        try {
            OverlayPackInstaller.InstalledOverlay? installed = OverlayPackInstaller.Install(filePath);
            if (installed == null) {
            ...
            OverlayManifest manifest = installed.Manifest;
            OverlayPorter.ImportResult result = await OverlayPorter.ImportExtractedRouteAsync(
                installed.UnpackDir,
                _factory,
                manifest.Name,
            ...
            foreach (Widget repointed in result.RepointedWidgets) {
                Widget? tracked = await db.Widgets.FirstOrDefaultAsync(w => w.Id == repointed.Id);
                tracked?.HtmlPath = repointed.HtmlPath;
            }

            if (result is { MergedRouteId: { } mergedId, MergedRouteName: { } mergedName }) {
                Route? existing = await db.Routes.FirstOrDefaultAsync(r => r.Id == mergedId);
