...
public class BoolToProcessedTextConverter : IValueConverter {
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture) {
        return value as bool? ?? false ? "Processed" : "Not Processed";
    }
...
public class BoolToProcessedColorConverter : IValueConverter {
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture) {
        return value as bool? ?? false ? Brushes.LimeGreen : Brushes.OrangeRed;
    }
...
public class CommandDeletableToBoolVisibilityConverter : IValueConverter {
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture) {
        return value is not SubathonCommandType type || !type.IsControlTypeCommand();
    }
...
public class IsNotMetaCommandConverter : IValueConverter {
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture) {
        return value is not SubathonCommandType type
...
public class GreaterThanOneToVisibilityConverter : IValueConverter {
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture) {
        return value switch { double d => d > 1, int i => i > 1, _ => false };
    }
...
public class GreaterThanZeroToVisibilityConverter : IValueConverter {
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture) {
        return value switch { double d => d > 0, int i => i > 0, _ => false };
    }
...
public class EventTypeUserBindingConverter : IMultiValueConverter {
    public object? Convert(IList<object?> values, Type targetType, object? parameter, CultureInfo culture) {
        ...
        string? eventTypeMeta = values.Count > 4 ? values[4]?.ToString() : null;

        var text = $"User: {user}";

        if (values[1] is SubathonEventType eventType) {
            if (eventType is SubathonEventType.GoAffProOrder &&
                GoAffProOrderHelper.TryGetStore(eventTypeMeta, out GoAffProStore? store))
                return $"New {store.StoreName} Order";
            if (eventType is SubathonEventType.JuniperMerchSale)
                return $"New {user} Sale";
            if (eventType is SubathonEventType.ThroneCrowdGiftComplete or SubathonEventType.MakeShipSale
        ...
        if (values[1] is SubathonEventType eventType) {
            if (eventType == SubathonEventType.TwitchRaid) {
                ...
                val = (values.Count > 4 ? values[4]?.ToString() : null)
                     ...
                     eventType.GetSource() is SubathonEventSource.Throne or SubathonEventSource.TreatStream) {
                type = "";
            }
            else {
                type = curr;
...
public partial class CssColorStringToColorConverter : IValueConverter {
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture) {
        if (value is not string str || string.IsNullOrWhiteSpace(str))
            return Colors.White;
        try {
            Match rgbaMatch = IsRgbaColourParseRegex().Match(str.Trim());
...
public class CssVariableTypeOptionsConverter : IValueConverter {
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture) {
        return value is WidgetCssVariableType type ? type.GetOptions() : Array.Empty<string>();
    }
...
public partial class CssSizeValueConverter : IValueConverter {
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture) {
        ...
        Match match = IsNumberRegex().Match(str);
...
public class StringToBoolConverter : IValueConverter {
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture) {
        return bool.TryParse(value as string, out bool b) && b;
    }

    public object ConvertBack(object? value, Type targetType, object? parameter, CultureInfo culture) {
        return (value is bool ? value.ToString() : "False") ?? "False";
    }
}
...
public class VarTooltipConverter : IValueConverter {
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture) {
        switch (value) {
            case CssVariable e: {
                WidgetCssVariableType type = e.Type == WidgetCssVariableType.Default
                    ? WidgetCssVariableType.String
                    : e.Type;
                string description = string.IsNullOrWhiteSpace(e.Description) ? "" : $"\n{e.Description}";
                return $"{e.Name}\nType: {type}{description}";
            }
            case JsVariable r:
                string jsDescription = string.IsNullOrWhiteSpace(r.Description) ? "" : $"\n{r.Description}";
...
public class WidgetIsHtmlConverter : IValueConverter {
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture) {
        return value is WidgetType.Html;
    }
...
public class WidgetIsPackedConverter : IValueConverter {
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture) {
        return value is string path && WidgetPackPaths.TryResolve(path, out _, out _, out _, out _);
    }
...
public class WidgetHasUpdateConverter : IValueConverter {
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture) {
        return value is string path && WidgetPackInstaller.FindNewerVersion(path) != null;
    }
