...
public partial class MainWindow {
    ...
    private void UpdateConnectionStatusDot() {
        List<IntegrationConnection> counted = Utils.GetAllConnections().Where(c => !IsAggregateExempt(c)).ToList();
    ...
    private static IBrush GetAggregateBrush(IEnumerable<IntegrationConnection> connections) {
        List<IntegrationConnection> counted = connections.Where(c => !IsAggregateExempt(c)).ToList();
    ...
    private static void SyncMenuItems(ItemCollection target, ItemCollection source) {
        if (target.Count != source.Count) {
            ...
            source.Clear();
            target.Clear();
            foreach (object item in items) target.Add(item);
            return;
        }
    ...
    private void FillConnectionStatusMenu(ItemCollection items) {
        items.Clear();
        List<IntegrationConnection> connections = Utils.GetAllConnections().ToList();
        if (connections.Count == 0) {
            items.Add(new MenuItem { Header = "No integrations active", IsEnabled = false });
            return;
        }

        List<IGrouping<SubathonEventSource, IntegrationConnection>> bySource = connections
            .GroupBy(c => GetEffectiveSource(c.Source))
            .OrderBy(g => SubathonEventSourceHelper.GetSourceOrder(g.Key))
            .ToList();

        IOrderedEnumerable<IGrouping<SubathonSourceGroup, IGrouping<SubathonEventSource, IntegrationConnection>>>
            byGroup = bySource
                .GroupBy(g => g.Key.GetGroup())
                .OrderBy(gg => gg.Min(s => SubathonEventSourceHelper.GetSourceOrder(s.Key)));

        foreach (IGrouping<SubathonSourceGroup, IGrouping<SubathonEventSource, IntegrationConnection>> group in
                 byGroup) {
            ...
            foreach (IGrouping<SubathonEventSource, IntegrationConnection> source in group)
    ...
    private MenuItem BuildSourceItem(SubathonEventSource source, List<IntegrationConnection> connections) {
        ...
        IOrderedEnumerable<IntegrationConnection> ordered = connections
            .OrderBy(c => c.Service == c.Source.ToString() ? 0 : 1)
            .ThenBy(c => c.Service, StringComparer.OrdinalIgnoreCase);
        foreach (IntegrationConnection connection in ordered) {
            ...
            if (connection.Source == SubathonEventSource.GoAffPro
                && GoAffProStoreRegistry.TryGetByInternalName(connection.Service, out GoAffProStore? store)) {
    ...
    private MenuItem BuildKoFiItem(List<IntegrationConnection> connections) {
        IntegrationConnection? tunnel = connections.FirstOrDefault(c => c.Source == SubathonEventSource.KoFiTunnel);
        IntegrationConnection? socket =
            connections.FirstOrDefault(c => c is { Source: SubathonEventSource.KoFi, Service: "Socket" });
        ...
        MenuItem socketItem = MakeLeafItem("Socket (Legacy)",
    ...
    private MenuItem BuildPallyItem(List<IntegrationConnection> connections) {
        IntegrationConnection socket = connections.FirstOrDefault(c => c.Service == "Socket") ?? connections.First();
        IBrush brush = LeafBrush(socket);
        var item = new MenuItem {
    ...
    private MenuItem BuildDevTunnelsItem(List<IntegrationConnection> connections) {
        ...
        IntegrationConnection? cli = connections.FirstOrDefault(c => c.Service == "Cli");
        IntegrationConnection? login = connections.FirstOrDefault(c => c.Service == "Login");
        IntegrationConnection? tunnel = connections.FirstOrDefault(c => c.Service == "Tunnel");
    ...
    private MenuItem BuildObsItem(List<IntegrationConnection> connections) {
        IntegrationConnection? webSocket = connections.FirstOrDefault(c => c.Service == "OBS");
        IntegrationConnection? helper = connections.FirstOrDefault(c => c.Service == "HelperScript");
    ...
    private static SubathonEventSource GetEffectiveSource(SubathonEventSource source) {
        SubathonEventSource trueSource = EnumMetaCache.Get<EventSourceMetaAttribute>(source)?.TrueSource
    ...
    private static void SetParentHeader(MenuItem item, string text) {
        item.Header = text;
    }

    private static Ellipse MakeStatusDot(IBrush? brush) {
        return new Ellipse {
            Width = 9,
            Height = 9,
            Fill = brush,
