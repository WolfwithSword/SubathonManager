...
public partial class TangiaSettings : SettingsControl {
    ...
    protected internal override void LoadValues(AppDbContext db) {
        ...
        string key = secureStorage.GetOrDefault(StorageKeys.TangiaEventKey, string.Empty) ?? string.Empty;
        SuppressUnsavedChanges(() => EventKeyBox.Text = key);

        SubathonValue? tokensValue = db.SubathonValues.FirstOrDefault(sv =>
    ...
    public override bool UpdateValueSettings(AppDbContext db) {
        ...
        if (tokensValue != null && double.TryParse(TokensSecondsBox.Text, out double seconds) &&
            !seconds.Equals(Math.Round(tokensValue.Seconds * 100))) {
            tokensValue.Seconds = seconds / 100.0;
            hasUpdated = true;
        }

        if (tokensValue != null && double.TryParse(TokensPointsBox.Text, out double points) &&
    ...
    private async void GetKey_Click(object? sender, RoutedEventArgs e) {
        try {
            ...
            if (!TangiaService.TryParseEventKey((urlBox.Text ?? "").Trim(), out string eventKey)) {
    ...
    private void TestTangia_Click(object? sender, RoutedEventArgs e) {
        if (!long.TryParse(SimulateTangiaAmt.Text, out long amount)) return;
