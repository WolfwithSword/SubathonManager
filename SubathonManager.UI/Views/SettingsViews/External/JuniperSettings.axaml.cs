...
public partial class JuniperSettings : SettingsControl {
    ...
    public JuniperSettings() {
        ...
        Loaded += (_, _) => {
            ...
            UpdateStatus(Utils.GetConnection(SubathonEventSource.JuniperCreates,
                nameof(SubathonEventSource.JuniperCreates)));
        };
    ...
    private static string ProductUrl(JuniperProduct product) {
        return $"https://{JuniperStoreRegistry.GetStoreName(product.StoreId)}/p/{product.ProductId}";
    }
    ...
    private void ReloadTrackingRows() {
        ...
        foreach (JuniperProduct product in JuniperStoreRegistry.AllProducts()) {
            var meta = product.ProductId.ToString();
            SubathonValue? ov = overrides.FirstOrDefault(sv => sv.Meta == meta);
            TrackedValueRow row = TrackingRows.AddRow(product, ProductUrl(product), product.ProductName,
                ov != null ? $"{ov.Seconds}" : "",
                ov != null ? $"{ov.Points}" : "");
            DecorateRow(row);
        }
    ...
    private void OnProductUpdated(JuniperProduct product) {
        Dispatcher.UIThread.Post(() => {
            TrackedValueRow? row = TrackingRows.Rows.FirstOrDefault(r =>
    ...
    private static void RemoveProductRecord(AppDbContext db, JuniperProduct product) {
        ...
        JuniperProduct? dbRow = db.JuniperProducts.FirstOrDefault(p => p.ProductId == product.ProductId);
        if (dbRow != null) db.JuniperProducts.Remove(dbRow);

        List<SubathonValue> orphanValues = db.SubathonValues
            .Where(sv => sv.EventType == SubathonEventType.JuniperMerchSale && sv.Meta == meta).ToList();
        db.SubathonValues.RemoveRange(orphanValues);

        JuniperStore? storeRow = db.JuniperStores.Include(s => s.Products)
    ...
    private static void RefreshRowStatus(TrackedValueRow row) {
        ...
        if (row.Item is JuniperProduct product) {
            ...
            if (product.Valid) {
                ...
                ToolTip.SetTip(info.StoreText, $"{storeName}: {product.ProductId}");
            }
            else {
                info.StoreText.Text = $"{storeName} (error)";
                info.StoreText.Foreground = Brushes.IndianRed;
                ToolTip.SetTip(info.StoreText, "Product could not be tracked");
            }
        ...
        if (url.Length > 0 && !JuniperStoreRegistry.TryParseProductUrl(url, out _, out _)) {
            ...
            info.StoreText.Foreground = Brushes.IndianRed;
            ToolTip.SetTip(info.StoreText, "Must look like https://<store>/p/<product id>");
        }
        else {
            info.StoreText.Text = "Not saved";
            info.StoreText.Foreground = Brushes.Gray;
            ToolTip.SetTip(info.StoreText, null);
        }
    }
    ...
    private bool SaveTrackings(AppDbContext db) {
        ...
        List<SubathonValue> juniperValues = db.SubathonValues
            .Where(sv => sv.EventType == SubathonEventType.JuniperMerchSale)
            .ToList();
        ...
        foreach (TrackedValueRow row in TrackingRows.Rows) {
            ...
            if (!JuniperStoreRegistry.TryParseProductUrl(url, out string storeName, out BigInteger productId)
            ...
            if (existing == null || existing.ProductId != productId) {
                ...
                JuniperStore store = GetOrCreateStore(db, storeName);
                ...
                JuniperProduct? dbRow = db.JuniperProducts.FirstOrDefault(p => p.ProductId == existing.ProductId);
        ...
        foreach (SubathonValue orphan in juniperValues.Where(sv => !validMetas.Contains(sv.Meta)).ToList()) {
    ...
    private JuniperStore GetOrCreateStore(AppDbContext db, string storeName) {
        JuniperStore? store = db.JuniperStores.Include(s => s.Products).AsEnumerable()
    ...
    private static bool SaveOverride(AppDbContext db, List<SubathonValue> juniperValues,
        ...
        var meta = productId.ToString();
        List<SubathonValue> existing = juniperValues.Where(sv => sv.Meta == meta).ToList();
        ...
        SubathonValue? keep = existing.FirstOrDefault();
        if (keep == null) {
