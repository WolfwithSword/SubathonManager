...
public partial class WidgetBrowserDialog : Window {
    ...
    protected override void OnClosed(EventArgs e) {
        ...
        foreach (Bitmap bitmap in _previews.Values)
            try {
                bitmap.Dispose();
            }
            catch {
    ...
    private void LoadState() {
        try {
            ...
            var packed = StateValueHelper.Get<string>(db, StateKeys.WidgetBrowserCollapsed);
            foreach (string key in packed.Split('\n',
    ...
    private async Task RefreshAsync() {
        ...
        try {
            List<WidgetCatalogEntry> entries = await Task.Run(() => WidgetCatalog.RefreshAsync(_factory));

            _all.Clear();
            foreach (WidgetCatalogEntry entry in entries)
    ...
    private Bitmap? LoadPreview(string? path) {
        ...
        if (_previews.TryGetValue(path, out Bitmap? cached)) return cached;
    ...
    private void BuildTree() {
        ...
        List<CatalogItem> ordered = items.ToList();

        foreach (IGrouping<string, CatalogItem> sectionGroup in ordered
            ...
            foreach (IGrouping<string, CatalogItem> groupGroup in sectionGroup
                         .GroupBy(i => i.GroupTitle, StringComparer.OrdinalIgnoreCase)
                         .OrderBy(g => g.Key == CatalogItem.UngroupedName ? 1 : 0)
                         .ThenBy(g => g.Key, StringComparer.OrdinalIgnoreCase)) {
                var key = $"G:{sectionGroup.Key}|{groupGroup.Key}";
                ...
                foreach (CatalogItem item in groupGroup.OrderBy(i => i.Name, StringComparer.OrdinalIgnoreCase))
    ...
    private static CatalogItem? ItemFrom(object? sender) {
        return sender is Control { Tag: CatalogItem item } ? item : null;
    }
    ...
    private async void AddSelected_Click(object? sender, RoutedEventArgs e) {
        ...
        List<CatalogItem> selected = _all.Where(i => i.IsSelected).ToList();
        ...
        try {
            foreach (CatalogItem? item in from item in selected
            ...
            foreach (CatalogItem item in selected) item.IsSelected = false;
    ...
    private async void RefreshEntry_Click(object? sender, RoutedEventArgs e) {
        ...
        try {
            ...
            WidgetCatalogEntry? updated = await Task.Run(() => WidgetCatalog.RefreshEntryAsync(_factory, packPath));
    ...
    private void DropCachedPreview(string? path) {
        ...
        if (!_previews.Remove(path, out Bitmap? bitmap)) return;
