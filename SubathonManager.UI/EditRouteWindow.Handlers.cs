...
public partial class EditRouteWindow {
    ...
    private void WidgetDirtyBorder_Loaded(object? sender, RoutedEventArgs e) {
        ...
        UiHelpers.UpdateButtonPendingBorder(border,
    ...
    private void UpdateWidgetCardDirty(Guid widgetId, bool isDirty) {
        if (!_widgetCardBorders.TryGetValue(widgetId, out Border? border)) return;
        if (!isDirty)
    ...
    private async Task SaveAllPendingWidgetChangesAsync() {
        ...
        List<Guid> allDirtyIds = _unsavedCssVars.Keys.Union(_unsavedJsVars.Keys).ToList();
        if (allDirtyIds.Count == 0) return;

        await using AppDbContext db = await _factory.CreateDbContextAsync();

        foreach (Guid widgetId in allDirtyIds) {
            ...
            if (_unsavedCssVars.TryGetValue(widgetId, out List<CssVariable>? stashedCss)) {
                foreach (CssVariable s in stashedCss) {
                    CssVariable? t = widget.CssVariables.Find(v => v.Name == s.Name);
            ...
            if (_unsavedJsVars.TryGetValue(widgetId, out List<JsVariable>? stashedJs)) {
                foreach (JsVariable s in stashedJs) {
                    JsVariable? t = widget.JsVariables.Find(v => v.Name == s.Name);
                    if (t != null) t.Value = s.Value;
                }
        ...
        if (_selectedWidget != null) {
            ...
            Widget? refreshed = await db.Widgets
                .Include(w => w.CssVariables)
                .Include(w => w.JsVariables)
                .FirstOrDefaultAsync(w => w.Id == _selectedWidget.Id);
            if (refreshed != null) {
                foreach (CssVariable v in refreshed.CssVariables) _editingCssVars.Add(v);
                foreach (JsVariable v in refreshed.JsVariables) _editingJsVars.Add(v);
            }
    ...
    private async void CopyOverlayUrl_Click(object? sender, RoutedEventArgs e) {
        try {
            ...
            await UiHelpers.TrySetClipboardTextAsync(_route.GetRouteUrl(config));
        }
        catch (Exception ex) {
    ...
    private async void EditorDrop(object? sender, DragEventArgs e) {
        ...
        List<string> paths = AcceptedDropPaths(e);
    ...
    private List<string> AcceptedDropPaths(DragEventArgs e) {
        ...
        try {
            ...
            accepted.AddRange(from item in files
                select item.TryGetLocalPath()
                into path
                where !string.IsNullOrWhiteSpace(path) && File.Exists(path)
                where DroppableExtensions.Contains(Path.GetExtension(path), StringComparer.OrdinalIgnoreCase)
                select path);
        }
        catch {
            /**/
    ...
    private bool IsOverWidgetEditorColumn(DragEventArgs e) {
        try {
            Point p = e.GetPosition(WidgetEditorColumn);
            Size size = WidgetEditorColumn.Bounds.Size;
    ...
    private void ReloadVars_Click(object? sender, RoutedEventArgs e) {
        ...
        UnsubscribeCssVarChanges();
        _editingCssVars.Clear();
        foreach (CssVariable v in _selectedWidget!.CssVariables) _editingCssVars.Add(v);
        SubscribeCssVarChanges();
    ...
    private async void SaveWidgetButton_Click(object? sender, RoutedEventArgs e) {
        try {
            ...
            _selectedWidget.ScaleX = float.TryParse(WidgetScaleXBox.Text, out float sx) ? sx :
                _selectedWidget.ScaleX == 0 ? 1 : _selectedWidget.ScaleX;
            _selectedWidget.ScaleY = float.TryParse(WidgetScaleYBox.Text, out float sy) ? sy :
                _selectedWidget.ScaleY == 0 ? 1 : _selectedWidget.ScaleY;

            foreach (CssVariable cssVar in _editingCssVars) {
                CssVariable? cssVarToUpdate = _selectedWidget.CssVariables.Find(csv => csv.Name == cssVar.Name);
            ...
            UnsubscribeCssVarChanges();
            _editingCssVars.Clear();
            foreach (CssVariable cssVar in _selectedWidget.CssVariables) _editingCssVars.Add(cssVar);

            SubscribeCssVarChanges();
            Widget? listEntry = _widgets.FirstOrDefault(wi => wi.Id == _selectedWidget.Id);
            if (listEntry != null) {
    ...
    private void OnWidgetActionRequested(Guid widgetId, WidgetContextAction action) {
        Dispatcher.UIThread.Post(async void () => {
            try {
                Widget? widget = _widgets.FirstOrDefault(w => w.Id == widgetId);
    ...
    private async Task ResetWidgetScaleAsync(Widget widget) {
        await using AppDbContext db = await _factory.CreateDbContextAsync();
        Widget? tracked = await db.Widgets.FirstOrDefaultAsync(w => w.Id == widget.Id);
    ...
    private async void ToggleVisibility_Click(object? sender, RoutedEventArgs e) {
        try {
            Widget? widget = GetWidgetFromSender(sender);
            if (widget == null) return;
    ...
    private async Task<bool> SetWidgetVisibilityAsync(Widget widget, bool visibility) {
        ...
        await using AppDbContext db = await _factory.CreateDbContextAsync();
        Widget? tracked = await db.Widgets.Include(w => w.CssVariables)
            .Include(w => w.JsVariables)
    ...
    private async void DeleteWidget_Click(object? sender, RoutedEventArgs e) {
        Widget? wi = GetWidgetFromSender(sender);
        if (wi == null) return;
    ...
    private void ShowWidgetErrorPopup(Widget widget) {
        ShowConfirmFlyout(WidgetAnchor(widget),
            ...
            "Delete Widget",
            true,
            () => _ = DeleteWidgetAsync(widget));
    }

    #region CARD POPUPS

    private Control WidgetAnchor(Widget widget) {
        return _widgetCardBorders.TryGetValue(widget.Id, out Border? border) ? border : this;
    }
    ...
    private IBrush TertiaryTextBrush() {
        return this.TryFindResource("UiBrushTextTertiary", ActualThemeVariant, out object? res) && res is IBrush brush
            ? brush
            : Brushes.Gray;
    }
    ...
    private void ShowConfirmFlyout(Control anchor, string title, string body, string? detail,
        ...
        StackPanel panel = BuildFlyoutPanel(title, body, detail);
    ...
    private async void ExportWidget_Click(object? sender, RoutedEventArgs e) {
        try {
            Widget? widget = (sender as Control)?.Tag as Widget ?? GetWidgetFromSender(sender);
    ...
    private void OpenWidgetFolder_Click(object? sender, RoutedEventArgs e) {
        ...
        string target = WidgetPackPaths.TryResolve(widget.HtmlPath, out string packFile, out _, out _, out _)
            ? packFile
            : widget.HtmlPath;
    ...
    private void UpdateWidgetVersion_Click(object? sender, RoutedEventArgs e) {
        ...
        Widget? widget = button.Tag as Widget ?? GetWidgetFromSender(sender);
        if (widget == null) return;

        if (!WidgetPackPaths.TryResolve(widget.HtmlPath, out _, out string entry, out _, out string version))
            return;

        WidgetPackInstaller.PackUpdate? update = WidgetPackInstaller.FindUpdate(widget.HtmlPath);
        ...
        ShowConfirmFlyout(button,
            ...
            "Update",
            false,
            () => _ = ApplyWidgetVersionUpdateAsync(widget, entry, update, button));
    }
    ...
    private void UnpackWidget_Click(object? sender, RoutedEventArgs e) {
        ...
        WidgetPackPaths.PackLocation? location = WidgetPackPaths.Resolve(widget.HtmlPath);
        if (location == null) return;

        if (!WidgetPackPaths.TryResolve(widget.HtmlPath, out _, out string entry, out _, out _))
            return;

        WidgetPackManifest? manifest = WidgetPackInstaller.ReadManifest(location.PackFileStr);
    ...
    private async Task ApplyUnpackAsync(Widget widget, string entry, string targetDir) {
        Control anchor = WidgetAnchor(widget);
    ...
    private async void CopyWidget_Click(object? sender, RoutedEventArgs e) {
        try {
            Widget? w = GetWidgetFromSender(sender);
            if (w == null) return;
    ...
    private async Task CloneWidgetAsync(Widget widget) {
        ...
        Widget clone = widget.Clone(widget.RouteId, widget.Name + " (Copy)", _widgets.Count + 1);
    ...
    private async Task SaveCurrentRoute() {
        ...
        try {
            ...
            if (int.TryParse(RouteWidthBox.Text, out int w)) _route.Width = w;
            if (int.TryParse(RouteHeightBox.Text, out int h)) _route.Height = h;
    ...
    private ComboBox? FindSiblingUnitBox(TextBox tb) {
        return tb.Parent is not Panel parent ? null : parent.Children.OfType<ComboBox>().FirstOrDefault();
    }
    ...
    private void ResetVars_Click(object? sender, RoutedEventArgs e) {
        ...
        _editingCssVars.Clear();
        _editingJsVars.Clear();
        foreach (CssVariable v in widget.CssVariables) _editingCssVars.Add(v);
        foreach (JsVariable v in widget.JsVariables) _editingJsVars.Add(v);
        SubscribeCssVarChanges();
        PopulateJsVars();
    ...
    private void SubscribeCssVarChanges() {
        foreach (CssVariable v in _editingCssVars) v.PropertyChanged += OnEditingCssVarChanged;
    ...
    private void OnEditingCssVarsCollectionChanged(object? sender,
        ...
        if (e.OldItems != null)
            foreach (CssVariable v in e.OldItems)
                v.PropertyChanged -= OnEditingCssVarChanged;
        if (e.NewItems != null)
            foreach (CssVariable v in e.NewItems)
                v.PropertyChanged += OnEditingCssVarChanged;
    }
    ...
    private void JsEventTypePickerBtn_Click(object? sender, RoutedEventArgs e) {
        ...
        string current = jsVar.Value ?? string.Empty;
        var entries = new List<EventTypeMenuEntry>();

        IOrderedEnumerable<SubathonEventType> values = Enum.GetValues<SubathonEventType>()
            .Where(x => x.IsEnabled())
            .Where(x => ((SubathonEventType?)x).HasNoValueConfig())
            .OrderBy(x => x.GetOrderNumber());

        foreach (SubathonEventType val in values) {
            ...
            SubathonEventType captured = val;
            entries.Add(new EventTypeMenuEntry(
                val.GetSource(), val.GetLabel(), current == captured.ToString(),
                () => SetJsEventTypeValue(btn, jsVar, captured.ToString())));
        }

        EventTypeMenu.Show(btn, entries, false,
            "- none -", () => SetJsEventTypeValue(btn, jsVar, string.Empty));
    }
    ...
    private void JsEventSubTypeSelectBox_Loaded(object? sender, RoutedEventArgs e) {
        ...
        if (cb.ItemCount == 0) {
            IOrderedEnumerable<SubathonEventSubType> values = Enum.GetValues<SubathonEventSubType>()
                .Where(x => ((SubathonEventSubType?)x).IsTrueEvent())
                .OrderBy(x => x.GetOrderNumber());
            cb.Items.Add(string.Empty);
            foreach (SubathonEventSubType val in values) cb.Items.Add(val.ToString());
            cb.SelectedItem = string.IsNullOrWhiteSpace(jsVar.Value) ? string.Empty : jsVar.Value;
            Dispatcher.UIThread.Post(() => { cb.SelectionChanged += (_, _) => jsVar.Value = $"{cb.SelectedItem}"; },
    ...
    private void JsStringSelectBox_Loaded(object? sender, RoutedEventArgs e) {
        if (sender is not ComboBox { Tag: JsVariable jsVar } cb) return;
        if (cb.ItemCount == 0) {
            string[] values = jsVar.Value?.Trim().Split(',') ?? [];
            foreach (string val in values) cb.Items.Add(val);
    ...
    private void JsFileVar_Loaded(object? sender, RoutedEventArgs e) {
        ...
        string shortContent = string.IsNullOrWhiteSpace(jsVar.Value) ? "Empty" : jsVar.Value.Split('/').Last();
        if (string.IsNullOrWhiteSpace(shortContent)) shortContent = "./";

        var valueBtn = new Button { Content = shortContent, Width = 150, Margin = new Thickness(0, 0, 0, 4) };
        ToolTip.SetTip(valueBtn, jsVar.Value);
        valueBtn.Click += async (_, _) => {
            string path = await SelectFileVarPathDialog(jsVar.Type);
        ...
        var openBtn = new Button {
            ...
            Width = 40, Height = 30, Margin = new Thickness(0, 0, 55, 2), Padding = new Thickness(2)
        };
        openBtn.Classes.Add("iconbtn");
        ToolTip.SetTip(openBtn, "Open");
        openBtn.Click += (_, _) => {
            string file = jsVar.Value;
    ...
    private void ApplyFileVarPath(JsVariable jsVar, Button valueBtn, string path) {
        ...
        if (ResourcePaths.ToResourceUrl(path) is { } resourceUrl) {
            ...
            string widgetDir = Path.GetDirectoryName(_selectedWidget.HtmlPath)!.Replace('\\', '/');
    ...
    private void SetFileVarDropHighlight(Button valueBtn, bool active) {
        if (!active) {
            valueBtn.ClearValue(BorderBrushProperty);
            return;
        }

        if (this.TryFindResource("AccentFillColorDefaultBrush",
                ActualThemeVariant, out object? res) && res is IBrush brush)
    ...
    private void JsEventTypeList_Loaded(object? sender, RoutedEventArgs e) {
        ...
        HashSet<string> panelValues = (jsVar.Value ?? "").Split(',',
                StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
            .ToHashSet(StringComparer.OrdinalIgnoreCase);

        var outerPanel = new StackPanel { Orientation = Orientation.Vertical };
        IOrderedEnumerable<IGrouping<SubathonEventSource, SubathonEventType>> groupValues = Enum
        ...
        foreach (IGrouping<SubathonEventSource, SubathonEventType> group in groupValues) {
            ...
            foreach ((string label, string key) in BuildEventTypeEntries(group)) {
                var chkBox = new CheckBox {
                    Content = new TextBlock
                        { Text = label, Tag = key, TextWrapping = TextWrapping.Wrap, MaxWidth = 240 },
    ...
    private static IEnumerable<(string Label, string Key)> BuildEventTypeEntries(IEnumerable<SubathonEventType> group) {
        foreach (SubathonEventType eType in group.OrderBy(x => x.GetOrderNumber())) {
    ...
    private void JsEventSubTypeList_Loaded(object? sender, RoutedEventArgs e) {
        ...
        var chkboxList = new StackPanel { Orientation = Orientation.Vertical };
        foreach (SubathonEventSubType eType in
                 Enum.GetValues<SubathonEventSubType>().OrderBy(x => x.GetOrderNumber())) {
            if (eType is SubathonEventSubType.CommandLike or SubathonEventSubType.Unknown) continue;
            var chkBox = new CheckBox {
                Content = new TextBlock
                    { Text = eType.GetDescription(), Tag = eType, TextWrapping = TextWrapping.Wrap, MaxWidth = 278 },
    ...
    private void JsPercentSlider_Loaded(object? sender, RoutedEventArgs e) {
        ...
        if (int.TryParse(jsVar.Value, out int initial)) slider.Value = initial;
    ...
    private void JsPercentBox_Loaded(object? sender, RoutedEventArgs e) {
        ...
        tb.Text = int.TryParse(jsVar.Value, out int initial) ? initial.ToString() : "0";

        tb.TextChanged += (_, _) => {
            if (string.IsNullOrWhiteSpace(tb.Text)) return;
            if (!int.TryParse(tb.Text, out int val)) return;
    ...
    private void JsFilteredEventTypeList_Loaded(object? sender, RoutedEventArgs e) {
        ...
        HashSet<SubathonEventType> allowedTypes = jsVar.Type.GetFilteredEventTypes().ToHashSet();
    ...
    private TextBox? FindPercentSiblingBox(Slider slider) {
        return slider.Parent is not Panel parent ? null : parent.Children.OfType<TextBox>().FirstOrDefault();
    }
