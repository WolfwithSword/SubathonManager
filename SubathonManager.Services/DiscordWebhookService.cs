...
public class DiscordWebhookService : IDisposable, IAppService {
    ...
    private readonly List<SubathonEventType> _auditEventTypes = new();
    private Task? _backgroundConfigTask;
    private Task? _backgroundTask;
    private Task? _backgroundTriggerTask;
    private Task? _backgroundWheelTask;
    private bool _doRemoteValuePatches;
    private bool _doSimulatedEvents;
    private bool _doWheelSpinEvents;
    private bool _doWheelTriggerEvents;
    ...
    public DiscordWebhookService(ILogger<DiscordWebhookService>? logger, IConfig config,
        CurrencyService currencyService, IHttpClientFactory? httpClientFactory = null) {
        _logger = logger;
        _config = config;
    ...
    public void LoadFromConfig() {
        ...
        foreach (SubathonEventType type in Enum.GetValues<SubathonEventType>()) {
            ...
            if (_config.GetBool("Discord", $"Events.Log.{type}"))
                _auditEventTypes.Add(type);
        }

        _auditGoAffProMetas.Clear();
        foreach (GoAffProStore store in GoAffProStoreRegistry.All())
            if (_config.GetBool("Discord", $"Events.Log.GoAffProOrder.{store.SiteId}"))
                _auditGoAffProMetas.Add(store.SiteId.ToString());

        _doSimulatedEvents = _config.GetBool("Discord", "Events.Log.Simulated");
        _doRemoteValuePatches = _config.GetBool("Discord", "Events.Log.RemoteConfig");
        _doWheelSpinEvents = _config.GetBool("Discord", "Wheel.Log.Enabled");
        _doWheelTriggerEvents = _config.GetBool("Discord", "Wheel.Log.Triggers");
    }
    ...
    private void OnSubathonConfigValuesPatched(List<SubathonValueDto> patched) {
        ...
        foreach (SubathonValueDto v in patched)
            _configQueue.Enqueue(v);
    }

    private string MakeUsername() {
        var username = $"{AppUsername} - {AppServices.AppVersion}";
    ...
    private void OnSubathonEventDeleted(List<SubathonEvent>? subathonEvents) {
        if (subathonEvents?.Count > 1 || subathonEvents is [{ Source: SubathonEventSource.Simulated }]) {
            ...
            double totalMoney = 0;
            string? currency = _config.Get("Currency", "Primary", "USD");
            foreach (SubathonEvent subathonEvent in subathonEvents) {
    ...
    private async Task FlushConfigQueueAsync() {
        ...
        while (events.Count < 5 * 10 && _configQueue.TryDequeue(out SubathonValueDto? sValue)) events.Add(sValue);
        if (events.Count == 0) return;

        foreach (List<SubathonValueDto> batch in events.Chunk(10)) {
    ...
    private async Task FlushQueueAsync() {
        ...
        while (events.Count < _maxMsgPerMinute * 10 && _eventQueue.TryDequeue(out SubathonEvent? subathonEvent))
        ...
        foreach (List<SubathonEvent> batch in events.Chunk(10)) {
            var embeds = batch.Select(e => new {
                title =
                    $"{GoAffProOrderHelper.GetOrderKey(e.EventType, e.EventTypeMeta)}{(e.Value.EndsWith(" [DELETED]") ? " - Deleted" : e.EventType == SubathonEventType.Command ? $" - {e.Command}" :
                        e.Source == SubathonEventSource.Simulated ? " - Simulated" : "")}",
                description = BuildEventDescription(e),
                color = e.ProcessedToSubathon && e.Value.EndsWith(" [DELETED]") ? 0x691911 :
                    e.ProcessedToSubathon ? 0x00ff88 : 0xffaa55,
    ...
    private string BuildEventDescription(SubathonEvent e) {
        ...
        if (e.EventType == SubathonEventType.JuniperMerchSale) sb.AppendLine($"**Store:** {e.User}");
        ...
        string val = e.Value.Replace("[DELETED]", "").Trim();
        if (e.Currency == "sub")
            val = val switch {
                "1000" => "Tier 1",
                "2000" => "Tier 2",
        ...
        if (e.Command is not (SubathonCommandType.Pause or SubathonCommandType.Resume or SubathonCommandType.Lock
            or SubathonCommandType.Unlock)) {
            ...
            if (e.Command != SubathonCommandType.SetMultiplier && e.Command != SubathonCommandType.StopMultiplier) {
                sb.AppendLine(
                    $"**Seconds:** {Math.Round(e.GetFinalSecondsValueRaw(), 2)} | **Points:** {e.GetFinalPointsValue()}");
                if (e.Command == SubathonCommandType.None)
                    sb.AppendLine($"**Multipliers:** x{e.MultiplierSeconds} time | x{e.MultiplierPoints} pts");
            }
        }

        if (e.EventType == SubathonEventType.TwitchGiftSub) sb.AppendLine($"**Amt:** x{e.Amount}");
        return sb.ToString();
    }
    ...
    private async Task SendWebhookAsync(object payload, string? url, int retryCount = 0) {
        ...
        try {
            HttpClient? http = _httpClientFactory?.CreateClient(nameof(DiscordWebhookService));
            if (http == null) return;
            string json = JsonSerializer.Serialize(payload);
            var content = new StringContent(json, Encoding.UTF8, "application/json");

            HttpResponseMessage response = await http.PostAsync(url, content, _cts.Token);
            var rc = "";
            ...
            if (response.StatusCode == HttpStatusCode.TooManyRequests && retryCount < 3) {
                ...
                var retryAfter = 1.0;
                try {
                    using JsonDocument doc = JsonDocument.Parse(rc);
                ...
                _logger?.LogWarning("Discord rate limited, retrying in {DelayMs}ms, attempt #{Attempt}", delayMs,
                    retryCount);
    ...
    private async Task FlushTriggerQueueAsync() {
        ...
        while (entries.Count < _maxMsgPerMinute * 10 && _triggerQueue.TryDequeue(out TriggerLogEntry entry))
            entries.Add(entry);
        if (entries.Count == 0) return;

        foreach (List<TriggerLogEntry> batch in entries.Chunk(10)) {
            var embeds = batch.Select(e => new {
    ...
    private async Task FlushWheelQueueAsync() {
        ...
        while (entries.Count < _maxMsgPerMinute * 10 && _wheelQueue.TryDequeue(out WheelLogEntry entry))
            entries.Add(entry);
        if (entries.Count == 0) return;

        foreach (List<WheelLogEntry> batch in entries.Chunk(10)) {
            var embeds = batch.Select(e => new {
    ...
    private string BuildWheelLogDescription(WheelLogEntry e) {
        ...
        sb.AppendLine(
            $"**Quantity Remaining:** {(e.Item?.IsInfinite ?? false ? "∞" : e.Item?.Quantity.ToString() ?? "?")}");
        WheelSpinAction? action = e.Item?.Action;
    ...
    private static int WheelStatusColor(WheelSpinHistoryStatus status) {
    ...
    private readonly record struct WheelLogEntry(
        string WheelName,
        WheelItem? Item,
        WheelSpinHistory History,
        int SpinsOwed,
        bool IsStatusChange);

    private readonly record struct TriggerLogEntry(
        WheelSpinTrigger Trigger,
        WheelSpinTriggerHistory History,
        int NewSpinsOwed);
}
