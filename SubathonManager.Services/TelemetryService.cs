...
public class TelemetryService(ILogger<TelemetryService>? logger, IConfig config, ITimerService? timerService = null)
    ...
    private static string? TelemetryKey =>
        "BUILD_TELEMETRY_KEY".StartsWith("BUILD")
            ? Environment.GetEnvironmentVariable("SM_TELEMETRY_SECRET")
            : "BUILD_TELEMETRY_KEY";
    ...
    private async Task TryPingAsync(CancellationToken ct = default) {
        if (!config.GetBool("Telemetry", "Enabled")) return;
        ...
        try {
            ...
            logger?.LogDebug("Telemetry sent for install id {InstallId}. Response: {ResponseCode}",
                config.GetInstallId(), response.StatusCode);
        }
        catch {
            /**/
        }
