...
public class PromptOrchestratorService(
    ...
    private async Task TryStartSchedulerAsync() {
        ...
        SubathonPromptRun? activeRun = await db.SubathonPromptRuns
            .Include(r => r.LinkedPrompt)
            .FirstOrDefaultAsync(r => r.Status == SubathonPromptRunStatus.Active);
        ...
        if (activeRun != null) {
            if (activeRun.IsExpired) {
                ...
                TimeSpan remaining = activeRun.TimeRemaining();
                timerService.Unregister(KeyInterval);
                timerService.Unregister(KeyCooldown);
                timerService.Register(KeyDuration, remaining, async _ => await OnDurationElapsedAsync(activeRun.Id));
                _logger?.LogInformation("[PromptOrchestrator] Resumed active prompt run {Id}, {Remaining} remaining",
                    activeRun.Id, remaining);
            }
    ...
    private void StartIntervalTimer(SubathonPromptSet set) {
        ...
        TimeSpan interval = BuildInterval(set);
    ...
    private TimeSpan BuildInterval(SubathonPromptSet set) {
        ...
        TimeSpan result = set.Interval + TimeSpan.FromSeconds(offsetSeconds);
    ...
    private async Task OnIntervalElapsedAsync() {
        ...
        await using AppDbContext db = await factory.CreateDbContextAsync();
        SubathonPromptSet? set = await db.SubathonPromptSets.Include(s => s.Prompts)
            .FirstOrDefaultAsync(s => s.IsActive);
        ...
        SubathonPrompt chosen = pickable[_rng.Next(pickable.Count)];
    ...
    private async Task StartRunAsync(AppDbContext db, SubathonPromptSet set, SubathonPrompt prompt) {
        ...
        _logger?.LogInformation("[PromptOrchestrator] Starting prompt run: {Text} (expires {Expires})", prompt.Text,
            run.ExpiresAt);
    ...
    private async void SubathonEventProcessed(SubathonEvent ev, bool processed) {
        try {
            ...
            await using AppDbContext db2 = await factory.CreateDbContextAsync();
            ...
            if (prompt is { IsInfinite: false, Quantity: > 0 }) {
                await using AppDbContext dbQty = await factory.CreateDbContextAsync();
                SubathonPrompt? trackedPrompt = await dbQty.SubathonPrompts.FindAsync(prompt.Id);
            ...
            SubathonPromptSet? set = await db.SubathonPromptSets.FindAsync(run.SetId);
    ...
    private void StartCooldownOrInterval(SubathonPromptSet? set) {
        ...
        if (set.Cooldown > TimeSpan.Zero) {
            ...
            timerService.Register(KeyCooldown, set.Cooldown, async ct => {
                ...
                SubathonPromptSet? refreshedSet = await db.SubathonPromptSets.Include(s => s.Prompts)
                    .FirstOrDefaultAsync(s => s.Id == set.Id, ct);
    ...
    private async void OnRunNowRequested(Guid promptId) {
        try {
            ...
            SubathonPromptRun? existing = await db.SubathonPromptRuns
                .FirstOrDefaultAsync(r => r.Status == SubathonPromptRunStatus.Active);
            if (existing != null) {
                timerService.Unregister(KeyDuration);
            ...
            SubathonPrompt? prompt = await db.SubathonPrompts.FindAsync(promptId);
            SubathonPromptSet? set = await db.SubathonPromptSets.Include(s => s.Prompts)
                .FirstOrDefaultAsync(s => s.IsActive);
    ...
    private static long GetEventDelta(SubathonEvent ev, SubathonPrompt prompt) {
        return prompt.Type switch {
            ...
            SubathonPromptType.Tokens => long.TryParse(ev.Value, out long t) ? t : 0,
    ...
    private static async Task<long> CountEventsAsync(AppDbContext db, IEnumerable<SubathonEventType> types) {
        List<SubathonEventType?> typeList = types.Cast<SubathonEventType?>().ToList();
    ...
    private static async Task<long> CountTokensAsync(AppDbContext db) {
        List<SubathonEventType?> types = SubathonEventSubTypeHelper.TokenEventTypes.Cast<SubathonEventType?>().ToList();
        List<string> rows = await db.SubathonEvents.AsNoTracking()
            .Where(e => e.ProcessedToSubathon && types.Contains(e.EventType))
            .Select(e => e.Value)
            .ToListAsync();
        return rows.Sum(v => long.TryParse(v, out long n) ? n : 0);
    }

    private static async Task<long> CountOrdersAsync(AppDbContext db, SubathonPrompt prompt) {
        List<SubathonEventType?> types = SubathonEventSubTypeHelper.OrderEventTypes.Cast<SubathonEventType?>().ToList();
    ...
    private static async Task<long> CountSubsAsync(AppDbContext db, SubathonPrompt prompt) {
        List<SubathonEventType?> subTypes = SubathonEventSubTypeHelper.SubEventTypes.Cast<SubathonEventType?>()
            .Where(e => prompt.SubType == SubathonPromptSubType.GiftSubs
                ? e.IsGift()
                : prompt.SubType != SubathonPromptSubType.NormalSubs || !e.IsGift()).ToList();
        IQueryable<SubathonEvent> query = db.SubathonEvents.AsNoTracking();
    ...
    private static async Task<long> CountEventTypeAsync(AppDbContext db, SubathonPrompt prompt) {
        ...
        SubathonEventType? filterType = prompt.FilterEventType;
        IQueryable<SubathonEvent> query = db.SubathonEvents.AsNoTracking()
        ...
        if (filterType is SubathonEventType.GoAffProOrder or SubathonEventType.JuniperMerchSale
            ...
            if (filterType == SubathonEventType.JuniperMerchSale &&
                Guid.TryParse(prompt.FilterMeta, out Guid storeId)) {
                List<string> productMetas = JuniperStoreRegistry.TryGetStore(storeId, out JuniperStore? store)
