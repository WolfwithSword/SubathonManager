...
public class TimerService : IDisposable, IAppService, ITimerService {
    ...
    public IDisposable Register(string key, TimeSpan interval, Action callback) {
        return Register(key, interval, _ => {
            callback();
            return Task.CompletedTask;
        });
    }
    ...
    public void Unregister(string key) {
        lock (_lock) {
            if (_scheduledActions.Remove(key)) _logger?.LogInformation("Removed {Key} timed action", key);
        }
    }
    ...
    private async Task TickAsync() {
        ...
        lock (_lock) {
            snapshot = [.. _scheduledActions.Values];
        }

        foreach (ScheduledAction action in snapshot) {
    ...
    private void Unregister(string key, ScheduledAction expected) {
        lock (_lock) {
            if (_scheduledActions.TryGetValue(key, out ScheduledAction? current) &&
