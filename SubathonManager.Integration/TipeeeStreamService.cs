...
public class TipeeeStreamService(
    ...
    private async Task InitializeAsync(CancellationToken ct = default) {
        ...
        if (string.IsNullOrWhiteSpace(_username)) {
            string? username = await FetchUsernameAsync(ct);
        ...
        if (string.IsNullOrWhiteSpace(ApiKey)) {
            string? apiKey = await FetchApiKeyAsync(ct);
            if (string.IsNullOrWhiteSpace(apiKey)) {
    ...
    private void OnNewEvent(SocketIOResponse response) {
        try {
            HandleParsedEvent(response.GetValue<TipeeeWrapper>()?.Event);
        }
        catch (Exception ex) {
    ...
    private void HandleParsedEvent(TipeeeEvent? ev) {
        ...
        TipeeeEventParameters? p = ev.Parameters;
        if (p == null) return;

        string refStr = ev.Ref.ValueKind == JsonValueKind.String
            ? ev.Ref.GetString() ?? ""
            : ev.Ref.ValueKind == JsonValueKind.Number
                ? ev.Ref.GetDouble().ToString(CultureInfo.InvariantCulture)
                : "";
        ...
        bool isSimulation = refStr.StartsWith("simulation", StringComparison.OrdinalIgnoreCase);

        string? username = isSimulation
            ? "SYSTEM"
        ...
        double amount = p.Amount.ValueKind == JsonValueKind.String
            ? double.TryParse(p.Amount.GetString(), NumberStyles.Any,
        ...
        var subathonEvent = new SubathonEvent {
            ...
            EventTimestamp = DateTimeOffset.TryParse(ev.CreatedAt, CultureInfo.InvariantCulture,
    ...
    public static void SimulateDonation(string amount, string currency) {
        if (!double.TryParse(amount, NumberStyles.Any,
                CultureInfo.InvariantCulture, out double amt)) return;
    ...
    private async Task<string?> FetchUsernameAsync(CancellationToken ct = default) {
        try {
            ...
            HttpResponseMessage resp = await client.GetAsync($"{ApiBase}/v1.0/me", ct);
            ...
            if (doc.RootElement.TryGetProperty("datas", out JsonElement datas)
                && datas.TryGetProperty("username", out JsonElement un))
                return un.GetString();
            if (doc.RootElement.TryGetProperty("username", out JsonElement rootUn))
    ...
    private async Task<string?> FetchApiKeyAsync(CancellationToken ct = default) {
        try {
            ...
            client.DefaultRequestHeaders.Authorization =
                new AuthenticationHeaderValue("Bearer", AccessToken);
            HttpResponseMessage resp = await client.GetAsync($"{ApiBase}/v1.0/me/api", ct);
            if (!resp.IsSuccessStatusCode) {
            ...
            using JsonDocument doc = JsonDocument.Parse(json);
            if (doc.RootElement.TryGetProperty("datas", out JsonElement datas)
                && datas.TryGetProperty("apiKey", out JsonElement key))
                return key.GetString();
            if (doc.RootElement.TryGetProperty("apiKey", out JsonElement rootKey))
    ...
    private async Task<string?> GetSocketUrlAsync(CancellationToken ct = default) {
        try {
            using HttpClient client = httpClientFactory.CreateClient(nameof(TipeeeStreamService));
            HttpResponseMessage resp = await client.GetAsync($"{ApiBase}/v2.0/site/socket", ct);
            if (!resp.IsSuccessStatusCode) {
            ...
            JsonElement dataEl = root.TryGetProperty("datas", out JsonElement d) ? d : root;
            {
                string? host = dataEl.TryGetProperty("host", out JsonElement h) ? h.GetString() : null;
                string? port = dataEl.TryGetProperty("port", out JsonElement p) ? p.GetString() : null;
    ...
    private async Task HandleApiKeyFailureAsync(CancellationToken ct = default) {
        ...
        string? newKey = await FetchApiKeyAsync(ct);
    ...
    private class TipeeeWrapper {
        [JsonPropertyName("event")] public TipeeeEvent? Event { get; set; }
    }
    ...
    private class TipeeeEvent {
        ...
        [JsonPropertyName("parameters")] public TipeeeEventParameters? Parameters { get; set; }

        [JsonPropertyName("user")] public TipeeeEventUser? User { get; set; }
    }
    ...
    private class TipeeeEventUser {
        ...
        [JsonPropertyName("providers")] public List<TipeeeEventProvider>? Providers { get; set; }
    }
