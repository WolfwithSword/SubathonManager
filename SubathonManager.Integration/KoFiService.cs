...
public class KoFiService(
    ...
    public async Task StartAsync(CancellationToken ct = default) {
        ...
        string?
            token = secureStorage.Get(StorageKeys
                .KoFiVerificationToken); // config.GetFromEncoded(_configSection, "VerificationToken", string.Empty);
    ...
    public async Task HandleWebhookAsync(byte[] rawBody, IReadOnlyDictionary<string, string> headers,
        ...
        string? token = secureStorage.Get(StorageKeys.KoFiVerificationToken);
        ...
        WebhookHandleResult<KoFiWebhookEvent> result =
            await _handler.HandleAsync(request, new KoFiWebhookOptions { VerificationToken = token }, ct);
            ...
            return koFiEvent switch {
                ...
                KoFiShopOrderEvent shop => new SubathonEvent {
                    ...
                    Value = sourceMode switch {
                        ...
                        _ => shop.Amount.ToString("F2", CultureInfo.InvariantCulture)
                    },
    ...
    private async Task ForwardRequestAsync(byte[] rawBody, IReadOnlyDictionary<string, string> headers,
        ...
        string[] urls = forwardUrlsRaw
            .Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);

        HttpClient client = httpClientFactory.CreateClient(nameof(KoFiService));
            ...
            try {
                ...
                foreach ((string key, string value) in headers) {
                    ...
                    if (key.Equals("Content-Type", StringComparison.OrdinalIgnoreCase)) {
                        if (MediaTypeHeaderValue.TryParse(value, out MediaTypeHeaderValue? mt))
