...
public class ThroneService(ILogger<ThroneService>? logger, IConfig config, DevTunnelsService devTunnels)
    ...
    private bool Enabled => config.GetBool("Throne", "Enabled");
    ...
    public Task HandleWebhookAsync(byte[] rawBody, IReadOnlyDictionary<string, string> headers,
        CancellationToken ct = default) {
        if (!Enabled) return Task.CompletedTask;
        if (!headers.TryGetValue("X-Signature-Timestamp", out string? timestamp)
            || string.IsNullOrWhiteSpace(timestamp)
            || !long.TryParse(timestamp, out long timestampUnix)) {
            logger?.LogWarning("[Throne] Rejected - missing or non-numeric X-Signature-Timestamp");
            return Task.CompletedTask;
        }

        if (!headers.TryGetValue("X-Signature-Ed25519", out string? signatureHex)
            || string.IsNullOrWhiteSpace(signatureHex)) {
            logger?.LogWarning("[Throne] Rejected - missing X-Signature-Ed25519");
            return Task.CompletedTask;
        }

        TimeSpan age = DateTimeOffset.UtcNow - DateTimeOffset.FromUnixTimeSeconds(timestampUnix);
        ...
        string stringData = Encoding.UTF8.GetString(rawBody);
    ...
    public void ProcessData(string stringData, bool isSim = false) {
        ...
        throneEvent.TryGetValue("data", out object? value);
        ...
        data.TryGetValue("is_surprise_gift", out object? isSurpriseRaw);
        ...
        throneEvent.TryGetValue("event_type", out object? eventType);
        subathonEvent.TertiaryValue = itemName?.ToString() ?? "New Gift";
        switch (eventType?.ToString()) {
            case "gift_purchased":
                OrderTypeModes mode = config.GetOrderTypeMode("Throne", $"{SubathonEventType.ThroneGiftPurchase}",
                    OrderTypeModes.Dollar);

                data.TryGetValue("price", out object? price);
                ...
                subathonEvent.Value = mode != OrderTypeModes.Dollar
                    ? itemName?.ToString() ?? "1"
                    : (priceInt / 100).ToString("F2", CultureInfo.InvariantCulture);
                break;
            case "contribution_purchased":
                ...
                subathonEvent.Value =
                    (amountInt / 100).ToString("F2", CultureInfo.InvariantCulture);
                break;
            case "gift_crowdfunded":
                data.TryGetValue("price", out object? price2);
