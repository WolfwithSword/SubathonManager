...
public class JuniperService(
    ILogger<JuniperService>? logger,
    ...
    public Task StartAsync(CancellationToken ct = default) {
        ...
        List<BigInteger> productIds = JuniperStoreRegistry.AllValidProductIds();
    ...
    internal static List<string> MakeQueryUrls(IReadOnlyList<BigInteger> productIds, DateTime startTime) {
        ...
        var start = startTime.ToUniversalTime().ToString("yyyy-MM-dd'T'HH:mm:ss.fff'Z'");
        for (var i = 0; i < productIds.Count; i += MaxIdsPerRequest) {
    ...
    internal async Task PollAsync(CancellationToken ct) {
        ...
        DateTime startTime = _lastFetchTime;
        _lastFetchTime = DateTime.UtcNow;
        ...
        var anyError = false;
        foreach (string url in MakeQueryUrls(productIds, startTime)) {
    ...
    internal bool HandleOrdersResponse(string json, DateTime windowStart) {
        try {
            ...
            if (root.ValueKind != JsonValueKind.Object ||
                !root.TryGetProperty("products_details", out JsonElement products) ||
            ...
            foreach (JsonProperty product in products.EnumerateObject()) {
                ...
                if (!product.Value.TryGetProperty("units_sold", out JsonElement unitsEl) ||
                    !unitsEl.TryGetInt32(out int unitsSold))
                    continue;

                if (unitsSold <= 0) continue;

                if (!JuniperStoreRegistry.TryGetProduct(productId, out JuniperProduct? juniperProduct)) continue;
    ...
    public static void Simulate(string meta, int count) {
        if (count <= 0) count = 1;
        meta = meta?.Trim() ?? "";
        string name = JuniperOrderHelper.TryGetProduct(meta, out JuniperProduct? product)
            ? product.ProductName
            : "Test Product";
    ...
    private void BroadcastStatus(bool force = false) {
        IReadOnlyList<JuniperStore> stores = JuniperStoreRegistry.AllStores();
    ...
    private async Task<string?> GetStringAsync(string url, CancellationToken ct) {
        using HttpClient client = httpClientFactory.CreateClient(nameof(JuniperService));
