...
public class TangiaService(
    ILogger<TangiaService>? logger,
    IHttpClientFactory httpClientFactory,
    ISecureStorage secureStorage) : IAppService {
    ...
    internal async Task PollOnceAsync(string key, CancellationToken ct) {
        ...
        var url = $"{PollBase}?key={Uri.EscapeDataString(key)}&cacheBust={cacheBust:F16}&instance={_instanceId}";

        using HttpClient client = httpClientFactory.CreateClient(nameof(TangiaService));
        ...
        foreach (TangiaEvent ev in payload.Events) {
    ...
    private async Task PingAsync(string key, CancellationToken ct) {
        try {
            var url = $"{PingBase}?key={Uri.EscapeDataString(key)}&instance={_instanceId}&currentEvent=undefined";
            using HttpClient client = httpClientFactory.CreateClient(nameof(TangiaService));
    ...
    public static bool TryParseEventKey(string overlayUrl, out string eventKey) {
        ...
        try {
            ...
            string? lastSegment = path.Split('/').LastOrDefault(s => !string.IsNullOrWhiteSpace(s));
    ...
    private class TangiaResponse {
        [JsonPropertyName("Events")] public List<TangiaEvent>? Events { get; set; }
    }

    internal class TangiaEvent {
        [JsonPropertyName("UnixMS")] public long UnixMs { get; set; }

        [JsonPropertyName("Data")] public TangiaEventData? Data { get; set; }
    ...
    internal class TangiaEventData {
        ...
        [JsonPropertyName("OverlayParams")] public TangiaOverlayParams? OverlayParams { get; set; }
    }

    internal class TangiaOverlayParams {
        [JsonPropertyName("InteractionTitle")] public string? InteractionTitle { get; set; }

        [JsonPropertyName("TriggerData")] public TangiaTriggerData? TriggerData { get; set; }

        [JsonPropertyName("BuyerInfo")] public TangiaBuyerInfo? BuyerInfo { get; set; }
    ...
    internal class TangiaBuyerInfo {
        [JsonPropertyName("has-plus")] public bool? HasPlus { get; set; }
