...
public class OverlayPorterTests : IDisposable {
    ...
    private string MakeTempWidget(string folderName, string htmlFileName = "widget.html") {
        string dir = Path.Combine(Path.GetTempPath(), "OverlayPorterTests", Guid.NewGuid().ToString(), folderName);
    ...
    private string MakeSmoFile(object manifest, Dictionary<string, string>? extraFiles = null) {
        string dir = Path.Combine(Path.GetTempPath(), "OverlayPorterTests", Guid.NewGuid().ToString());
        Directory.CreateDirectory(dir);
        _tempDirs.Add(dir);
        string smoPath = Path.Combine(dir, "test.smo");

        using ZipArchive zip = ZipFile.Open(smoPath, ZipArchiveMode.Create);

        string manifestJson = JsonSerializer.Serialize(manifest, new JsonSerializerOptions {
            WriteIndented = true,
            Converters = { new JsonStringEnumConverter() }
        });
        ZipArchiveEntry entry = zip.CreateEntry("overlay.json");
        ...
        if (extraFiles != null)
            foreach ((string zipPath, string content) in extraFiles) {
                ZipArchiveEntry e = zip.CreateEntry(zipPath);
    ...
    private string MakeTempExtractRoot() {
        string dir = Path.Combine(Path.GetTempPath(), "OverlayPorterTests", Guid.NewGuid().ToString(), "imports");
        Directory.CreateDirectory(dir);
        _tempDirs.Add(dir);
        return dir;
    }
    ...
    public void GetZipWidgetRoots_EmptyList_ReturnsEmpty() {
        List<string> result = OverlayPorter.GetZipWidgetRoots(new List<string>());
        Assert.Empty(result);
    }
    ...
    public void GetZipWidgetRoots_SameParent_SharesHash() {
        ...
        List<string> result = OverlayPorter.GetZipWidgetRoots(paths);

        Assert.Equal(2, result.Count);

        string[] timerParts = result[0].Split('/');
        string[] alertsParts = result[1].Split('/');
    ...
    public void GetZipWidgetRoots_DifferentDrives_GetDifferentHashs() {
        ...
        Assert.Equal(2, result.Count);
        string[] cParts = result[0].Split('/');
        string[] gParts = result[1].Split('/');
    ...
    public void GetZipWidgetRoots_SharedPrefixWithDifferentLeafs_SameParentDifferentLeafsAsRoots_SameHash() {
        ...
        string[] p1 = result[0].Split('/');
        string[] p2 = result[1].Split('/');
    ...
    public void GetZipWidgetRoots_ThreePaths_TwoSameDriveOneDifferent() {
        ...
        string cBucket = result[0].Split('/')[1];
        string g1Bucket = result[1].Split('/')[1];
        string g2Bucket = result[2].Split('/')[1];
    ...
    public void GetZipWidgetRoots_TwoPaths_TwoDrivesOtherwiseSame() {
        ...
        Assert.Equal(2, result.Count);
        string g1Bucket = result[0].Split('/')[1];
        string g2Bucket = result[1].Split('/')[1];
    ...
    public void GetZipWidgetRoots_SanitisesSegments_TheSameOnEveryPlatform() {
        List<string> result = OverlayPorter.GetZipWidgetRoots([@"C:\stream\my:timer"]);
    ...
    public void GetZipWidgetRoots_ReservedSegmentName_IsEscaped() {
        List<string> result = OverlayPorter.GetZipWidgetRoots([@"C:\stream\CON"]);
    ...
    public void GetZipWidgetRoots_ResultCountMatchesInputCount() {
        List<string> paths = Enumerable.Range(0, 5)
    ...
    public async Task ExportRouteAsync_CreatesZipFile() {
        SetupServices();
        string htmlPath = MakeTempWidget("mytimer");
        ...
        string outDir = Path.Combine(Path.GetTempPath(), "OverlayPorterTests", Guid.NewGuid().ToString());
        Directory.CreateDirectory(outDir);
        _tempDirs.Add(outDir);
        string outPath = Path.Combine(outDir, "test.smo");
    ...
    public async Task ExportRouteAsync_ZipContainsManifest() {
        ...
        string outPath = Path.Combine(Path.GetTempPath(), "OverlayPorterTests",
        ...
        await using ZipArchive zip = await ZipFile.OpenReadAsync(outPath, TestContext.Current.CancellationToken);
    ...
    public async Task ExportRouteAsync_ManifestContainsWidgetHtmlPath() {
        ...
        await (await zip.GetEntry("overlay.json")!.OpenAsync(TestContext.Current.CancellationToken)).CopyToAsync(ms,
            TestContext.Current.CancellationToken);
        JsonDocument doc = JsonDocument.Parse(ms.ToArray());
        List<JsonElement> widgets = doc.RootElement.GetProperty("widgets").EnumerateArray().ToList();

        Assert.Single(widgets);
        string htmlZipPath = widgets[0].GetProperty("htmlPath").GetString()!;
    ...
    public async Task ExportRouteAsync_ManifestUsesExportName() {
        ...
        await (await zip.GetEntry("overlay.json")!.OpenAsync(TestContext.Current.CancellationToken)).CopyToAsync(ms,
            TestContext.Current.CancellationToken);
        JsonDocument doc = JsonDocument.Parse(ms.ToArray());
        string? name = doc.RootElement.GetProperty("route").GetProperty("name").GetString();
    ...
    public async Task ExportRouteAsync_ExcludedEntries_NotInZip() {
        SetupServices();
        string htmlPath = MakeTempWidget("mytimer");
        string assetPath = Path.Combine(Path.GetDirectoryName(htmlPath)!, "bg.png");
        ...
        List<string> zipRoots = OverlayPorter.GetZipWidgetRoots(new List<string>
    ...
    public async Task ExportRouteAsync_WidgetFilesIncludedInZip() {
        SetupServices();
        string htmlPath = MakeTempWidget("mytimer");
        string extraFile = Path.Combine(Path.GetDirectoryName(htmlPath)!, "style.css");
    ...
    public async Task ImportRouteAsync_MissingManifest_ReturnsFailed() {
        IDbContextFactory<AppDbContext> factory = SetupServices();
        string dir = Path.Combine(Path.GetTempPath(), "OverlayPorterTests", Guid.NewGuid().ToString());
        Directory.CreateDirectory(dir);
        _tempDirs.Add(dir);

        string smoPath = Path.Combine(dir, "empty.smo");
        await using (ZipArchive zip =
                     await ZipFile.OpenAsync(smoPath, ZipArchiveMode.Create, TestContext.Current.CancellationToken)) {
            zip.CreateEntry("dummy.txt");
        }

        OverlayPorter.ImportResult result =
            await OverlayPorter.ImportRouteAsync(smoPath, MakeTempExtractRoot(), factory);
    ...
    public async Task ImportRouteAsync_NewRoute_RouteIsNew() {
        ...
        string smo = MakeSmoFile(manifest, new Dictionary<string, string> {
            [htmlZipPath] = "<html></html>"
        });

        OverlayPorter.ImportResult result = await OverlayPorter.ImportRouteAsync(smo, MakeTempExtractRoot(), factory);
    ...
    public async Task ImportRouteAsync_NewRoute_WidgetInNewWidgets() {
        ...
        string smo = MakeSmoFile(manifest, new Dictionary<string, string> { [htmlZipPath] = "" });
        OverlayPorter.ImportResult result = await OverlayPorter.ImportRouteAsync(smo, MakeTempExtractRoot(), factory);

        Assert.Single(result.NewWidgets);
        Widget w = result.NewWidgets[0];
    ...
    public async Task ImportRouteAsync_NewRoute_WidgetHtmlPathPointsInsideExtractDir() {
        ...
        string extractRoot = MakeTempExtractRoot();
        string smo = MakeSmoFile(manifest, new Dictionary<string, string> { [htmlZipPath] = "" });
        OverlayPorter.ImportResult result = await OverlayPorter.ImportRouteAsync(smo, extractRoot, factory);

        Widget widget = result.NewWidgets[0];
    ...
    public async Task ImportRouteAsync_ReImport_RouteNotNew_NoDuplicateWidgets() {
        ...
        //seed
        OverlayPorter.ImportResult first = await OverlayPorter.ImportRouteAsync(smo, extractRoot, factory);
        ...
        OverlayPorter.ImportResult second = await OverlayPorter.ImportRouteAsync(smo, extractRoot, factory);
    ...
    public async Task ImportRouteAsync_ReImport_NewVariable_AddedToExistingWidget() {
        ...
        object MakeManifest(object[] jsVars) {
            return new {
                version = 1,
                route = new { name = "R", resolution = new { width = 1920, height = 1080 } },
                widget_folder_map = new { },
        ...
        string smo1 = MakeSmoFile(MakeManifest([]), new Dictionary<string, string> { [htmlZipPath] = "" });
        OverlayPorter.ImportResult first = await OverlayPorter.ImportRouteAsync(smo1, extractRoot, factory);
        await using (AppDbContext db = await factory.CreateDbContextAsync(TestContext.Current.CancellationToken)) {
            db.Routes.Add(first.Route!);
            db.Widgets.AddRange(first.NewWidgets);
            await db.SaveChangesAsync(TestContext.Current.CancellationToken);
        ...
        string smo2 = MakeSmoFile(MakeManifest([
            new { name = "myVar", value = "hello", type = WidgetVariableType.String }
        ]), new Dictionary<string, string> { [htmlZipPath] = "" });
        OverlayPorter.ImportResult second = await OverlayPorter.ImportRouteAsync(smo2, extractRoot, factory);
    ...
    public async Task ImportRouteAsync_CssVariables_ImportedOnNewWidget() {
        ...
        Assert.Single(result.NewWidgets);
        List<CssVariable> cssVars = result.NewWidgets[0].CssVariables;
    ...
    public async Task ImportRouteAsync_RelativeFileVariable_ResolvedToAbsoluteInsideExtractDir() {
        ...
        JsVariable? jsVar = result.NewWidgets[0].JsVariables.FirstOrDefault(v => v.Name == "bgImage");
    ...
    public async Task RoundTrip_ExportThenImport_WidgetHtmlPathResolvesCorrectly() {
        IDbContextFactory<AppDbContext> factory = SetupServices();
        string htmlPath = MakeTempWidget("roundtrip");
        ...
        Directory.CreateDirectory(outDir);
        _tempDirs.Add(outDir);
        string smoPath = Path.Combine(outDir, "rt.smo");

        await OverlayPorter.ExportRouteAsync(route, smoPath, "RT Export");

        string extractRoot = MakeTempExtractRoot();
        OverlayPorter.ImportResult result = await OverlayPorter.ImportRouteAsync(smoPath, extractRoot, factory);

        Assert.False(result.Failed);
        Assert.True(result.RouteIsNew);
        Assert.Single(result.NewWidgets);

        Widget imported = result.NewWidgets[0];
    ...
    public async Task RoundTrip_ExportThenImport_WidgetPropertiesPreserved() {
        IDbContextFactory<AppDbContext> factory = SetupServices();
        string htmlPath = MakeTempWidget("props");
        ...
        Directory.CreateDirectory(outDir);
        _tempDirs.Add(outDir);
        string smoPath = Path.Combine(outDir, "props.smo");
    ...
    public async Task ExportRouteAsync_AbsoluteFileVariable_RewrittenToRelativeInManifest() {
        SetupServices();
        string htmlPath = MakeTempWidget("timerfx");

        string extDir = Path.Combine(Path.GetTempPath(), "OverlayPorterTests", Guid.NewGuid().ToString(), "sounds");
        Directory.CreateDirectory(extDir);
        _tempDirs.Add(Path.GetDirectoryName(extDir)!);
        string soundFile = Path.Combine(extDir, "alert.mp3");
        ...
        await (await zip.GetEntry("overlay.json")!.OpenAsync(TestContext.Current.CancellationToken)).CopyToAsync(ms,
            TestContext.Current.CancellationToken);
        JsonDocument doc = JsonDocument.Parse(ms.ToArray());
        List<JsonElement> jsVars = doc.RootElement
            .GetProperty("widgets")[0]
            .GetProperty("jsVariables")
            .EnumerateArray()
            .ToList();

        JsonElement alertVar = jsVars.FirstOrDefault(v => v.GetProperty("name").GetString() == "alertSound");
        Assert.True(alertVar.ValueKind != JsonValueKind.Undefined, "alertSound variable not found in manifest");
        string rewrittenValue = alertVar.GetProperty("value").GetString()!;
    ...
    public async Task ExportRouteAsync_AbsoluteFileVariable_FileIsInExternalFolderInZip() {
        SetupServices();
        string htmlPath = MakeTempWidget("timerfx2");

        string extDir = Path.Combine(Path.GetTempPath(), "OverlayPorterTests", Guid.NewGuid().ToString(), "assets");
        Directory.CreateDirectory(extDir);
        _tempDirs.Add(Path.GetDirectoryName(extDir)!);
        string imageFile = Path.Combine(extDir, "logo.png");
    ...
    public async Task ExportRouteAsync_FolderPathVariable_AllFilesInExternalSubfolder() {
        SetupServices();
        string htmlPath = MakeTempWidget("timerFolder");

        string extDir = Path.Combine(Path.GetTempPath(), "OverlayPorterTests", Guid.NewGuid().ToString(), "media");
        Directory.CreateDirectory(extDir);
        _tempDirs.Add(Path.GetDirectoryName(extDir)!);
        ...
        await (await zip.GetEntry("overlay.json")!.OpenAsync(TestContext.Current.CancellationToken)).CopyToAsync(ms,
            TestContext.Current.CancellationToken);
        JsonDocument doc = JsonDocument.Parse(ms.ToArray());
        JsonElement folderVar = doc.RootElement
            .GetProperty("widgets")[0]
            .GetProperty("jsVariables")
            .EnumerateArray()
    ...
    public async Task ExportRouteAsync_RelativeFileVariable_NotMovedToExternal() {
        SetupServices();
        string htmlPath = MakeTempWidget("relvar");
        string assetPath = Path.Combine(Path.GetDirectoryName(htmlPath)!, "asset.png");
        await File.WriteAllBytesAsync(assetPath, [0x89, 0x50], TestContext.Current.CancellationToken);
    ...
    public async Task ImportRouteAsync_ReImport_NewCssVariable_AddedToExistingWidget() {
        ...
        object MakeManifest(object[] cssVars) {
            return new {
                version = 1,
                route = new { name = "R", resolution = new { width = 1920, height = 1080 } },
                widget_folder_map = new { },
