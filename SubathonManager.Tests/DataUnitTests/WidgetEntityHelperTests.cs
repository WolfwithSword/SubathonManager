...
public class WidgetEntityHelperTests {
    ...
    private static string CreateTempHtmlWithCss(string cssContent, out string cssPath) {
        ...
        string htmlPath = Path.Combine(dir, "widget.html");
    ...
    private static string CreateTempHtmlWithCssAndMeta(string cssContent, string metaJson, out string cssPath) {
        string htmlPath = CreateTempHtmlWithCss(cssContent, out cssPath);
    ...
    public void LoadNewJsVariables_CreatesAndUpdatesVariables() {
        ...
        (List<JsVariable> added, List<string> names, List<JsVariable> updated) =
            helper.LoadNewJsVariables(widget, helper.ConvertHtmlMetaToJsonMeta(metadata));

        // new vars
        Assert.Contains(added, v => v is { Name: "newVar", Value: "val1" });

        Assert.Contains(updated,
            v => v.Name == "selectVar" && v.Value.Contains("b,c,a")); // order preserved, a appended
    ...
    public void ExtractWidgetMetadata_ReturnsCorrectDictionary() {
        ...
        var helper = new WidgetEntityHelper(null, null);
        WidgetMeta dict = helper.ExtractWidgetMetadataSync(tempFile);

        Assert.Equal(3, dict.Vars.Count);
        dict.Vars.TryGetValue("key1", out WidgetMetaVar? value);
        Assert.Equal("value1", value!.ValueToString());
        dict.Vars.TryGetValue("key2", out WidgetMetaVar? value2);
        Assert.Equal("value2", value2!.ValueToString());
        dict.Vars.TryGetValue("key3", out WidgetMetaVar? value3);
    ...
    public async Task ExtractWidgetMetadataAsync_ReturnsCorrectDictionary() {
        ...
        var helper = new WidgetEntityHelper(null, null);
        WidgetMeta meta = await helper.ExtractWidgetMetadata(tempFile);

        Assert.Single(meta.Vars);
        WidgetMetaVar valueA = meta.Vars["keyA"];
    ...
    public void SyncCssVariables_AddsNewVariables() {
        ...
        string htmlPath = CreateTempHtmlWithCss("--test-color: #fff;", out _);
        ...
        using AppDbContext db2 = factory.CreateDbContext();
        List<CssVariable> vars = db2.CssVariables.Where(v => v.WidgetId == widget.Id).ToList();
        Assert.Single(vars);
    ...
    public void SyncCssVariables_RemovesStaleVariables() {
        IDbContextFactory<AppDbContext> factory = SetupServices();
        string htmlPath = CreateTempHtmlWithCss("--keep: red;", out _);
    ...
    public void SyncCssVariables_UpdatesTypeAndDescription_WhenChanged() {
        ...
        var metaJson = """
                       ...
                       """;
        string htmlPath = CreateTempHtmlWithCssAndMeta("--my-var: #000;", metaJson, out _);
        ...
        using AppDbContext db2 = factory.CreateDbContext();
        CssVariable? updated = db2.CssVariables.FirstOrDefault(v => v.WidgetId == widget.Id && v.Name == "my-var");
    ...
    public void SyncCssVariables_DeduplicatesVariables() {
        IDbContextFactory<AppDbContext> factory = SetupServices();
        string htmlPath = CreateTempHtmlWithCss("--dup: 1px;", out _);
        ...
        using AppDbContext db2 = factory.CreateDbContext();
        List<CssVariable> vars = db2.CssVariables.Where(v => v.WidgetId == widget.Id && v.Name == "dup").ToList();
        Assert.Single(vars);
    ...
    public void SyncCssVariables_NoHtmlFile_DoesNotThrow() {
        ...
        var helper = new WidgetEntityHelper(factory, null);
        Exception? ex = Record.Exception(() => helper.SyncCssVariables(widget));
        Assert.Null(ex);
    ...
    public void SyncJsVariables_AddsNewVariables() {
        IDbContextFactory<AppDbContext> factory = SetupServices();
        string htmlPath = CreateTempHtmlWithMeta("myVar.String:hello");
        ...
        using AppDbContext db2 = factory.CreateDbContext();
        List<JsVariable> vars = db2.JsVariables.Where(v => v.WidgetId == widget.Id).ToList();
    ...
    public void SyncJsVariables_RemovesStaleVariables() {
        IDbContextFactory<AppDbContext> factory = SetupServices();
        string htmlPath = CreateTempHtmlWithMeta("keepVar.String:yes");
    ...
    public void SyncJsVariables_UpdatesDocsUrl_WhenValidUrlInMeta() {
        IDbContextFactory<AppDbContext> factory = SetupServices();
        string htmlPath = CreateTempHtmlWithMeta("Url: https://example.com/docs\nmyVar.String: val");
    ...
    public void SyncJsVariables_DoesNotUpdateDocsUrl_WhenInvalid() {
        IDbContextFactory<AppDbContext> factory = SetupServices();
        string htmlPath = CreateTempHtmlWithMeta("Url: not-a-url\nmyVar.String:val");
    ...
    public void SyncJsVariables_UpdatesStringSelectValues_PreservingOrder() {
        IDbContextFactory<AppDbContext> factory = SetupServices();
        string htmlPath = CreateTempHtmlWithMeta("selectVar.StringSelect:a,b,c");
        ...
        using AppDbContext db2 = factory.CreateDbContext();
        JsVariable? updated = db2.JsVariables.FirstOrDefault(v => v.WidgetId == widget.Id && v.Name == "selectVar");
        Assert.NotNull(updated);
    ...
    public void SyncJsVariables_DeduplicatesVariables() {
        IDbContextFactory<AppDbContext> factory = SetupServices();
        string htmlPath = CreateTempHtmlWithMeta("dupVar.String:x");
        ...
        using AppDbContext db2 = factory.CreateDbContext();
        List<JsVariable> vars = db2.JsVariables.Where(v => v.WidgetId == widget.Id && v.Name == "dupVar").ToList();
        Assert.Single(vars);
    ...
    public void SyncJsVariables_NoneValue_StoredAsEmpty() {
        IDbContextFactory<AppDbContext> factory = SetupServices();
        string htmlPath = CreateTempHtmlWithMeta("myVar.String:NONE");
        ...
        using AppDbContext db2 = factory.CreateDbContext();
        JsVariable? v = db2.JsVariables.FirstOrDefault(x => x.WidgetId == widget.Id && x.Name == "myVar");
        Assert.NotNull(v);
    ...
    public async Task UpdateWidgetPosition_ReturnsFalse_WhenDataIsEmpty() {
        IDbContextFactory<AppDbContext> factory = SetupServices();
        Widget widget = await SeedWidgetAsync(factory);
        var helper = new WidgetEntityHelper(factory, null);

        bool result = await helper.UpdateWidgetPosition(widget.Id.ToString(), new Dictionary<string, JsonElement>());
    ...
    public async Task UpdateWidgetPosition_ReturnsFalse_WhenGuidIsInvalid() {
        ...
        bool result = await helper.UpdateWidgetPosition("not-a-guid", data);
    ...
    public async Task UpdateWidgetPosition_ReturnsFalse_WhenWidgetNotFound() {
        ...
        bool result = await helper.UpdateWidgetPosition(Guid.NewGuid().ToString(), data);
    ...
    public async Task UpdateWidgetPosition_UpdatesX_WhenProvided() {
        ...
        bool result = await helper.UpdateWidgetPosition(widget.Id.ToString(), data);

        Assert.True(result);
        await using AppDbContext db = await factory.CreateDbContextAsync(TestContext.Current.CancellationToken);
        Widget? updated = await db.Widgets.FindAsync([widget.Id], TestContext.Current.CancellationToken);
    ...
    public async Task UpdateWidgetPosition_UpdatesZ_WhenProvided() {
        ...
        await using AppDbContext db = await factory.CreateDbContextAsync(TestContext.Current.CancellationToken);
        Widget? updated =
            await db.Widgets.FindAsync(new object?[] { widget.Id }, TestContext.Current.CancellationToken);
    ...
    public async Task UpdateWidgetScale_IgnoresUnrecognisedKeys() {
        ...
        bool result = await helper.UpdateWidgetScale(widget.Id.ToString(), data);
    ...
    public async Task UpdateWidgetScale_ReturnsFalse_WhenDataIsEmpty() {
        ...
        bool result = await helper.UpdateWidgetScale(widget.Id.ToString(), new Dictionary<string, JsonElement>());
    ...
    public async Task UpdateWidgetScale_ReturnsFalse_WhenGuidIsInvalid() {
        ...
        bool result = await helper.UpdateWidgetScale("not-a-guid", data);
    ...
    public async Task UpdateWidgetScale_ReturnsFalse_WhenWidgetNotFound() {
        ...
        bool result = await helper.UpdateWidgetScale(Guid.NewGuid().ToString(), data);
