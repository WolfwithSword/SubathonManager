...
public class WidgetPackReaderTests {
    private static WidgetPackReader MakeReader(TempWorkspace ws, string packFile) {
        return new WidgetPackReader(packFile, Path.Combine(ws.Root, "cache", Guid.NewGuid().ToString("N")));
    }

    private static string MakePack(TempWorkspace ws, params (string entry, string content)[] files) {
        return TestPacks.WriteZip(ws.Path_("packs", $"{Guid.NewGuid():N}.smw"),
    ...
    public void MissingPackFile_BehavesLikeAnEmptyArchive() {
        ...
        WidgetPackReader reader = MakeReader(ws, Path.Combine(ws.Root, "nope.smw"));
    ...
    public void CorruptPackFile_BehavesLikeAnEmptyArchive() {
        using var ws = new TempWorkspace("packreader");
        WidgetPackReader reader = MakeReader(ws, ws.WriteFile("broken.smw", "not a zip at all"));
    ...
    public void Entries_AreNormalisedAndDirectoriesSkipped() {
        ...
        WidgetPackReader reader = MakeReader(ws, pack);
        List<string> entries = reader.Entries.ToList();
    ...
    public void Contains_NormalisesTheLookupKey(string probe) {
        using var ws = new TempWorkspace("packreader");
        WidgetPackReader reader = MakeReader(ws, MakePack(ws, ("content/widget.html", "<html></html>")));
    ...
    public void Contains_UnknownEntry_IsFalse() {
        using var ws = new TempWorkspace("packreader");
        WidgetPackReader reader = MakeReader(ws, MakePack(ws, ("content/widget.html", "x")));
    ...
    public void LengthOf_KnownEntry_ReturnsUncompressedLength() {
        using var ws = new TempWorkspace("packreader");
        WidgetPackReader reader = MakeReader(ws, MakePack(ws, ("a.txt", "0123456789")));
    ...
    public void Read_ReturnsTheEntryBytes() {
        using var ws = new TempWorkspace("packreader");
        WidgetPackReader reader = MakeReader(ws, MakePack(ws, ("a.txt", "hello")));
    ...
    public void ReadText_WithoutBom_ReturnsTextVerbatim() {
        using var ws = new TempWorkspace("packreader");
        WidgetPackReader reader = MakeReader(ws, MakePack(ws, ("a.txt", "héllo wörld")));
    ...
    public void Materialize_UnknownEntry_ReturnsNull() {
        using var ws = new TempWorkspace("packreader");
        WidgetPackReader reader = MakeReader(ws, MakePack(ws, ("a.txt", "x")));
    ...
    public void Materialize_SmallEntry_ReturnsNull_BecauseItIsCachedInMemory() {
        using var ws = new TempWorkspace("packreader");
        WidgetPackReader reader = MakeReader(ws, MakePack(ws, ("a.txt", "small")));
    ...
    public void ExtractAll_WritesEveryEntry() {
        using var ws = new TempWorkspace("packreader");
        WidgetPackReader reader = MakeReader(ws, MakePack(ws,
            ("widget.json", "{}"),
            ("content/widget.html", "<html></html>"),
            ("content/sub/style.css", "body{}")));
    ...
    public void ExtractAll_SkipsZipSlipEntries() {
        using var ws = new TempWorkspace("packreader");
        WidgetPackReader reader = MakeReader(ws, MakePack(ws, ("../escaped.txt", "pwned"), ("safe.txt", "fine")));
    ...
    public void ExtractAll_Twice_OverwritesCleanly() {
        using var ws = new TempWorkspace("packreader");
        WidgetPackReader reader = MakeReader(ws, MakePack(ws, ("a.txt", "content")));
        string target = Path.Combine(ws.Root, "out");
...
public class WidgetPackMemoryCacheTests {
    private static string Key(string suffix) {
        return WidgetPackMemoryCache.MakeKey($"pack-{Guid.NewGuid():N}", suffix);
    }
    ...
    public void TryGet_Miss_ReturnsFalseAndEmptyArray() {
        ...
        Assert.False(WidgetPackMemoryCache.TryGet(Key("nope"), out byte[] data));
        Assert.Empty(data);
    }
    ...
    public void SetThenTryGet_RoundTrips() {
        ...
        Assert.True(WidgetPackMemoryCache.TryGet(key, out byte[] data));
    ...
    public void InvalidatePack_DropsOnlyThatPacksEntries() {
        WidgetPackMemoryCache.Clear();
        var mine = $"pack-{Guid.NewGuid():N}";
        var other = $"pack-{Guid.NewGuid():N}";
