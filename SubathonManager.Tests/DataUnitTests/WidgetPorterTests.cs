...
public class WidgetPorterPureTests {
    ...
    public void IsSelected_LockedOrDefaultSelected_IsTrue() {
        WidgetPorter.ExportPlan plan = PlanWith(
    ...
    public void IsSelected_IsCaseInsensitive() {
        WidgetPorter.ExportPlan plan = PlanWith(new WidgetPorter.SmwEntry
            { ZipEntry = "content/A.png", DefaultSelected = true });
    ...
    public void ResolveVarValue_OptionalRewrite_OnlyAppliesWhenTheEntryIsSelected() {
        WidgetPorter.ExportPlan plan = PlanWith(new WidgetPorter.SmwEntry
            { ZipEntry = "content/_external/resources/a.png" });
...
public class WidgetPorterWorkspaceTests {
    ...
    public void ReadExistingMeta_NoMetaFile_ReturnsDefaults() {
        using var ws = new TempWorkspace("porter");
        WidgetMeta meta = WidgetPorter.ReadExistingMeta(MakeLooseWidget(ws));
    ...
    public void ReadExistingMeta_MalformedJson_ReturnsDefaults() {
        using var ws = new TempWorkspace("porter");
        Widget widget = MakeLooseWidget(ws);
    ...
    public void ReadExistingMeta_ValidJson_IsDeserialised() {
        ...
        WidgetMeta meta = WidgetPorter.ReadExistingMeta(widget);
    ...
    public void BuildPlan_AssetWidget_ProducesAnEmptyPlan(WidgetType type) {
        ...
        WidgetPorter.ExportPlan plan = WidgetPorter.BuildPlan(widget);
    ...
    public void BuildPlan_AlwaysEmitsManifestEntryAndMeta() {
        using var ws = new TempWorkspace("porter");
        WidgetPorter.ExportPlan plan = WidgetPorter.BuildPlan(MakeLooseWidget(ws));

        Assert.Equal("content/widget.html", plan.EntryZipPath);
        WidgetPorter.SmwEntry manifest = plan.Entries.Single(e => e.Kind == WidgetPorter.SmwEntryKind.Manifest);
        Assert.Equal(WidgetPorter.ManifestFileName, manifest.ZipEntry);
        Assert.True(manifest.Locked);
        Assert.NotNull(manifest.Generator);

        WidgetPorter.SmwEntry entry = plan.Entries.Single(e => e.Kind == WidgetPorter.SmwEntryKind.Entry);
        Assert.Equal("content/widget.html", entry.ZipEntry);
        Assert.True(entry.Locked);
        Assert.Null(entry.Generator);

        WidgetPorter.SmwEntry meta = plan.Entries.Single(e => e.Kind == WidgetPorter.SmwEntryKind.Meta);
    ...
    public void BuildPlan_ExistingMetaFile_IsUsedAsTheSource() {
        ...
        WidgetPorter.SmwEntry meta = WidgetPorter.BuildPlan(widget).Entries
    ...
    public void BuildPlan_LinkedCssInsideTheWidgetFolder_KeepsItsRelativeLayout() {
        ...
        WidgetPorter.SmwEntry css = WidgetPorter.BuildPlan(widget).Entries
    ...
    public void BuildPlan_LinkedCssOutsideTheWidgetFolder_GoesToTheSharedFolder() {
        ...
        WidgetPorter.ExportPlan plan = WidgetPorter.BuildPlan(widget);
        WidgetPorter.SmwEntry css = plan.Entries.Single(e => e.Kind == WidgetPorter.SmwEntryKind.Css);

        Assert.Equal("content/_shared/common.css", css.ZipEntry);

        WidgetPorter.SmwEntry htmlEntry = plan.Entries.Single(e => e.Kind == WidgetPorter.SmwEntryKind.Entry);
    ...
    public void BuildPlan_LoneCssVariableClaimedOnce_AcrossSeveralStylesheets() {
        ...
        WidgetPorter.ExportPlan plan = WidgetPorter.BuildPlan(widget);
        List<WidgetPorter.SmwEntry> cssEntries =
            plan.Entries.Where(e => e.Kind == WidgetPorter.SmwEntryKind.Css).ToList();
    ...
    public void BuildPlan_OtherFilesInTheFolder_BecomeUnselectedAssets() {
        ...
        List<WidgetPorter.SmwEntry> assets = WidgetPorter.BuildPlan(widget).Entries
    ...
    public void BuildPlan_AbsoluteFileVariable_BecomesAnExternalEntryAndRewrite() {
        ...
        WidgetPorter.ExportPlan plan = WidgetPorter.BuildPlan(widget);
        WidgetPorter.SmwEntry external = plan.Entries.Single(e => e.Kind == WidgetPorter.SmwEntryKind.External);
    ...
    public void BuildPlan_AbsoluteFolderVariable_BundlesTheWholeFolder() {
        ...
        WidgetPorter.ExportPlan plan = WidgetPorter.BuildPlan(widget);
        List<WidgetPorter.SmwEntry> external =
            plan.Entries.Where(e => e.Kind == WidgetPorter.SmwEntryKind.External).ToList();
    ...
    public void BuildPlan_ResourceReferencedFromMarkup_IsMarkedInUse() {
        using var ws = new TempWorkspace("porter");
        ws.WriteFile("resources/images/logo.png", "png");
        Widget widget = MakeLooseWidget(ws, """<html><body><img src="/resources/images/logo.png"></body></html>""");

        WidgetPorter.SmwEntry external = WidgetPorter.BuildPlan(widget).Entries
    ...
    public async Task ExportWidgetAsync_WritesManifestEntryAndMeta() {
        ...
        await using ZipArchive zip = await ZipFile.OpenReadAsync(output, TestContext.Current.CancellationToken);
    ...
    public async Task ExportWidgetAsync_ManifestCarriesTheExportOptions() {
        ...
        await using ZipArchive zip = await ZipFile.OpenReadAsync(output, TestContext.Current.CancellationToken);
        JsonElement root = JsonDocument.Parse(Manifest(zip)).RootElement;
        JsonElement w = root.GetProperty("widget");
    ...
    public async Task ExportWidgetAsync_MetaJson_CarriesResolvedVariableValues() {
        ...
        await using ZipArchive zip = await ZipFile.OpenReadAsync(output, TestContext.Current.CancellationToken);
        JsonElement vars = JsonDocument.Parse(EntryText(zip, "content/widget.html.json"))
    ...
    public async Task ExportedPack_CanBeInstalledAndReadBack() {
        ...
        WidgetPackInstaller.InstalledPack? installed = WidgetPackInstaller.Install(output);
