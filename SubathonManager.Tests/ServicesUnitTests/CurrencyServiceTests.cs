...
public class CurrencyServiceTests {
    private static readonly string _usdRatesJson = """
                                                    {
                                                       "aud": {
                                                           "code": "AUD",
                                                           "alphaCode": "AUD",
    ...
    private readonly MockWebServerHost _mockWebServerHost =
        new MockWebServerHost().OnGet("/daily/usd.json", _usdRatesJson);
    ...
    public async Task ConvertAsync_ReturnsCorrectAmount() {
        ...
        CurrencyService service = CreateService(fakeJson);
    ...
    public async Task ConvertAsync_SameCurrency_ReturnsSameAmount() {
        ...
        double result = await service.ConvertAsync(10, "USD", "USD");
    ...
    public async Task FetchBaseAsync_UpdatesRatesAndPreventsConcurrentFetch() {
        var json = @"{ ""usd"": { ""code"": ""USD"", ""rate"": 1.0 }, ""eur"": { ""code"": ""EUR"", ""rate"": 0.9 } }";

        CurrencyService service = CreateService(json);
    ...
    public async Task StartAsync_LoadsFromFile_WhenFileExists() {
        ...
        CurrencyService service = CreateService("{}");
    ...
    public void IsExpired_ReturnsTrue_WhenFileOld() {
        ...
        MethodInfo method =
            typeof(CurrencyService).GetMethod("IsExpired", BindingFlags.NonPublic | BindingFlags.Instance)!;
        var expired = (bool)method.Invoke(service, null)!;
    ...
    public async Task GetValidCurrenciesAsync_IncludesDefaultCurrency() {
        CurrencyService service = CreateService("{}");
        List<string> list = await service.GetValidCurrenciesAsync();
