...
public class PromptOrchestratorServiceTests : IAsyncDisposable {
    ...
    private static bool CallEventMatchesPrompt(SubathonEvent ev, SubathonPrompt prompt) {
        MethodInfo method = typeof(PromptOrchestratorService)
    ...
    public void EventMatchesPrompt_Money_MatchesCurrencyDonation() {
        ...
        var ev = new SubathonEvent
            { EventType = SubathonEventType.StreamElementsDonation, Value = "10.00", Currency = "CAD" };
    ...
    public async Task GetCurrentCountAsync_Points_ReturnsSubathonPoints() {
        ...
        var prompt = new SubathonPrompt { Type = SubathonPromptType.Points };
        long result = await PromptOrchestratorService.GetCurrentCountAsync(db, prompt);
    ...
    public async Task GetCurrentCountAsync_Follows_CountsFollowEvents() {
        ...
        db.SubathonEvents.Add(new SubathonEvent
            { EventType = SubathonEventType.TwitchFollow, ProcessedToSubathon = true, Amount = 1 });
        db.SubathonEvents.Add(new SubathonEvent
            { EventType = SubathonEventType.TwitchFollow, ProcessedToSubathon = true, Amount = 1 });
        db.SubathonEvents.Add(new SubathonEvent
            { EventType = SubathonEventType.TwitchSub, ProcessedToSubathon = true, Amount = 1 }); // should not count
        await db.SaveChangesAsync(TestContext.Current.CancellationToken);
    ...
    public async Task GetCurrentCountAsync_Tokens_SumsValues() {
        ...
        db.SubathonDatas.Add(new SubathonData { IsActive = true, Points = 0, Currency = "USD" });
        db.SubathonEvents.Add(new SubathonEvent
            { EventType = SubathonEventType.TwitchCheer, ProcessedToSubathon = true, Value = "100", Amount = 100 });
        db.SubathonEvents.Add(new SubathonEvent
            { EventType = SubathonEventType.TwitchCheer, ProcessedToSubathon = true, Value = "250", Amount = 250 });
        await db.SaveChangesAsync(TestContext.Current.CancellationToken);
    ...
    public async Task GetCurrentCountAsync_Subs_SumsAmount() {
        ...
        db.SubathonDatas.Add(new SubathonData { IsActive = true, Points = 0, Currency = "USD" });
        db.SubathonEvents.Add(new SubathonEvent
            { EventType = SubathonEventType.TwitchSub, ProcessedToSubathon = true, Amount = 1 });
        db.SubathonEvents.Add(new SubathonEvent
            { EventType = SubathonEventType.TwitchGiftSub, ProcessedToSubathon = true, Amount = 5 });
        await db.SaveChangesAsync(TestContext.Current.CancellationToken);
    ...
    public async Task GetCurrentCountAsync_Subs_GiftOnly_ExcludesNormal() {
        ...
        db.SubathonEvents.Add(new SubathonEvent
            { EventType = SubathonEventType.TwitchSub, ProcessedToSubathon = true, Amount = 1 });
        db.SubathonEvents.Add(new SubathonEvent
            { EventType = SubathonEventType.TwitchGiftSub, ProcessedToSubathon = true, Amount = 3 });
        await db.SaveChangesAsync(TestContext.Current.CancellationToken);
    ...
    public async Task GetCurrentCountAsync_Orders_Default_CountsOrders() {
        ...
        db.SubathonDatas.Add(new SubathonData { IsActive = true, Points = 0, Currency = "USD" });
        db.SubathonEvents.Add(new SubathonEvent {
            EventType = SubathonEventType.GoAffProOrder, EventTypeMeta = "132230", ProcessedToSubathon = true,
            Amount = 1
        });
        db.SubathonEvents.Add(new SubathonEvent {
            EventType = SubathonEventType.GoAffProOrder, EventTypeMeta = "132230", ProcessedToSubathon = true,
    ...
    public async Task OnSubathonDataUpdate_ActiveUnpausedUnlocked_StartsScheduler() {
        (_, SubathonPromptSet set, _) = await SeedBasicAsync();
        var timer = new MockTimerService();
        PromptOrchestratorService svc = CreateService(timer);
    ...
    public async Task OnSubathonDataUpdate_DisabledSet_DoesNotRegisterInterval() {
        ...
        db.SubathonDatas.Add(new SubathonData
            { IsActive = true, IsPaused = false, IsLocked = false, Points = 0, Currency = "USD" });
    ...
    public async Task OnCancelRequested_ActiveRun_SetsStatusCancelled() {
        (_, SubathonPromptSet set, SubathonPrompt prompt) = await SeedBasicAsync();
        ...
        await using AppDbContext verifyDb = CreateDb();
        SubathonPromptRun? updated =
            await verifyDb.SubathonPromptRuns.FindAsync([run.Id], TestContext.Current.CancellationToken);
    ...
    public async Task OnRunNowRequested_CreatesNewActiveRun() {
        ...
        await using AppDbContext verifyDb = CreateDb();
        SubathonPromptRun? run = await verifyDb.SubathonPromptRuns
            .FirstOrDefaultAsync(r => r.PromptId == prompt.Id, TestContext.Current.CancellationToken);
        Assert.NotNull(run);
    ...
    public async Task OnRunNowRequested_CancelsExistingActiveRun() {
        ...
        await using (AppDbContext db = CreateDb()) {
            db.SubathonPromptRuns.Add(existingRun);
            await db.SaveChangesAsync(TestContext.Current.CancellationToken);
        }
        ...
        await using AppDbContext verifyDb = CreateDb();
        SubathonPromptRun? old =
            await verifyDb.SubathonPromptRuns.FindAsync([existingRun.Id], TestContext.Current.CancellationToken);
    ...
    public async Task SubathonEventProcessed_NotProcessed_IsIgnored() {
        ...
        await using AppDbContext verifyDb = CreateDb();
        SubathonPromptRun run = await verifyDb.SubathonPromptRuns.FirstAsync(TestContext.Current.CancellationToken);
    ...
    public async Task SubathonEventProcessed_ProgressBelowTarget_RaisesProgressEvent() {
        (_, SubathonPromptSet set, SubathonPrompt prompt) = await SeedBasicAsync(initialPoints: 50);
    ...
    public async Task SubathonEventProcessed_ProgressMeetsTarget_CompletesRun() {
        (_, SubathonPromptSet set, SubathonPrompt prompt) = await SeedBasicAsync(initialPoints: 100);
        var runId = Guid.NewGuid();
        await using (AppDbContext db = CreateDb()) {
            db.SubathonPromptRuns.Add(new SubathonPromptRun {
                Id = runId, PromptId = prompt.Id, SetId = set.Id,
        ...
        await using AppDbContext verifyDb = CreateDb();
        SubathonPromptRun? saved =
            await verifyDb.SubathonPromptRuns.FindAsync([runId], TestContext.Current.CancellationToken);
    ...
    public async Task SubathonEventProcessed_Completion_DecrementsQuantity() {
        (_, SubathonPromptSet set, SubathonPrompt prompt) = await SeedBasicAsync(
            SubathonPromptType.Points, 10, 3, initialPoints: 10);
        await using (AppDbContext db = CreateDb()) {
            db.SubathonPromptRuns.Add(new SubathonPromptRun {
                PromptId = prompt.Id, SetId = set.Id, Status = SubathonPromptRunStatus.Active,
                StartedAt = DateTime.Now, ExpiresAt = DateTime.Now.AddMinutes(5),
        ...
        await using AppDbContext verifyDb = CreateDb();
        SubathonPrompt? updatedPrompt =
            await verifyDb.SubathonPrompts.FindAsync([prompt.Id], TestContext.Current.CancellationToken);
    ...
    public async Task SubathonEventProcessed_Completion_InfinitePrompt_QuantityUnchanged() {
        (_, SubathonPromptSet set, SubathonPrompt prompt) = await SeedBasicAsync(
            SubathonPromptType.Points, 10, 5, true);
        await using (AppDbContext db = CreateDb()) {
    ...
    public async Task SubathonEventProcessed_EventDoesNotMatchPromptType_IsIgnored() {
        (_, SubathonPromptSet set, SubathonPrompt prompt) = await SeedBasicAsync(SubathonPromptType.Follows, 5);
        await using (AppDbContext db = CreateDb()) {
            db.SubathonDatas.Add(new SubathonData { IsActive = true, Points = 0, Currency = "USD" });
            db.SubathonPromptRuns.Add(new SubathonPromptRun {
                PromptId = prompt.Id, SetId = set.Id, Status = SubathonPromptRunStatus.Active,
    ...
    public async Task SubathonEventProcessed_ExpiredRun_IsIgnored() {
        ...
        await using AppDbContext verifyDb = CreateDb();
        SubathonPromptRun saved = await verifyDb.SubathonPromptRuns.FirstAsync(TestContext.Current.CancellationToken);
    ...
    public async Task TryStartScheduler_ActiveNonExpiredRun_ResumesDurationTimer() {
        (_, SubathonPromptSet set, SubathonPrompt prompt) = await SeedBasicAsync(duration: TimeSpan.FromMinutes(10));
        await using (AppDbContext db = CreateDb()) {
            db.SubathonDatas.Add(new SubathonData { IsActive = true, Points = 0, Currency = "USD" });
            db.SubathonPromptRuns.Add(new SubathonPromptRun {
                PromptId = prompt.Id, SetId = set.Id, Status = SubathonPromptRunStatus.Active,
    ...
    public async Task Cooldown_WhenConfigured_RegistersCooldownNotInterval() {
        (_, SubathonPromptSet set, SubathonPrompt prompt) = await SeedBasicAsync(cooldown: TimeSpan.FromMinutes(2));
        ...
        FieldInfo runningField = typeof(PromptOrchestratorService)
    ...
    public async Task IntervalElapsed_NoPickablePrompts_ReschedulesInterval() {
        (_, SubathonPromptSet set, _) = await SeedBasicAsync(promptEnabled: false, quantity: 0);
        await using (AppDbContext db = CreateDb()) {
            db.SubathonDatas.Add(new SubathonData { IsActive = true, Points = 0, Currency = "USD" });
            await db.SaveChangesAsync(TestContext.Current.CancellationToken);
        }
...
public class MockTimerService : ITimerService {
    ...
    public IDisposable Register(string key, TimeSpan delay, Func<CancellationToken, Task> callback) {
        lock (_lock) {
            _registered[key] = (delay, callback);
        }
    ...
    public async Task FireAsync(string key, CancellationToken ct = default) {
        ...
        lock (_lock) {
            if (!_registered.TryGetValue(key, out (TimeSpan Delay, Func<CancellationToken, Task> Callback) entry))
                return;
            cb = entry.Callback;
        }
