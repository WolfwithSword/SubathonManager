...
public class EnumValidationTests {
    ...
    public void AssertEnumGroups() {
        List<SubathonEventSource> streamSources = Enum.GetValues<SubathonEventSource>().Where(x =>
        ...
        List<SubathonEventSource> externalServices = Enum.GetValues<SubathonEventSource>().Where(x =>
        ...
        List<SubathonEventSource> extensionServices = Enum.GetValues<SubathonEventSource>().Where(x =>
        ...
        List<IGrouping<SubathonEventSource, SubathonEventType>> eventTypes =
            Enum.GetValues<SubathonEventType>().GroupBy(x => x.GetSource()).ToList();
        Assert.True(eventTypes.Any());
        foreach (IGrouping<SubathonEventSource, SubathonEventType> group in eventTypes) {
            if (group.Key == SubathonEventSource.GoAffPro) {
                foreach (SubathonEventType subathonEventType in group) {
                ...
                List<int> siteIds = group.Where(t => t != SubathonEventType.GoAffProOrder)
