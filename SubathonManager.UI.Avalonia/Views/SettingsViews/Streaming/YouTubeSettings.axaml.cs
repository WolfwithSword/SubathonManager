...
public partial class YouTubeSettings : SettingsControl
{
    ...
    public override bool UpdateValueSettings(AppDbContext db)
    {
        ...
        var superchatValue = db.SubathonValues.FirstOrDefault(sv => sv.EventType == SubathonEventType.YouTubeSuperChat && sv.Meta == "");
        if (superchatValue != null && double.TryParse(DonoBox.Text, out var scSeconds) && !scSeconds.Equals(superchatValue.Seconds))
        {
            superchatValue.Seconds = scSeconds;
            hasUpdated = true;
        }
        if (superchatValue != null && double.TryParse(DonoBox2.Text, out var scPoints) && !scPoints.Equals(superchatValue.Points))
        {
            superchatValue.Points = scPoints;
            hasUpdated = true;
        }

        var redirectValue = db.SubathonValues.FirstOrDefault(sv => sv.EventType == SubathonEventType.YouTubeRedirect && sv.Meta == "");
        if (redirectValue != null && double.TryParse(RaidBox.Text, out var rdSeconds) && !rdSeconds.Equals(redirectValue.Seconds))
        {
            redirectValue.Seconds = rdSeconds;
            hasUpdated = true;
        }
        if (redirectValue != null && double.TryParse(RaidBox2.Text, out var rdPoints) && !rdPoints.Equals(redirectValue.Points))
        {
