...
public static partial class WidgetPackPaths {
    ...
    public static string MountRoot(string packId, string version) {
        return Path.Combine(PackedRoot, packId, version);
    }

    public static string PackFile(string packId, string version) {
        return Path.Combine(PackedRoot, packId, version + PackExtension);
    }

    public static string PackFolder(string packId) {
        return Path.Combine(PackedRoot, packId);
    }

    public static List<string> InstalledVersions(string packId) {
        return VersionsIn(PackFolder(packId));
    }

    public static List<string> VersionsIn(string folder) {
        lock (VersionCacheLock) {
            if (VersionCache.TryGetValue(folder, out List<string>? cached)) return cached;
    ...
    public static int CompareVersions(string left, string right) {
        List<long> a = VersionSegments(left);
        List<long> b = VersionSegments(right);
    ...
    private static List<long> VersionSegments(string version) {
        return VersionSegmentRegex().Matches(version ?? string.Empty)
            .Select(m => long.TryParse(m.Value, out long n) ? n : 0L)
            .ToList();
    }

    public static string NormalizeGroup(string? group) {
        return string.IsNullOrWhiteSpace(group) ? DefaultGroup : group.Trim();
    }
    ...
    public static bool IsVersionName(string? name) {
        return !string.IsNullOrWhiteSpace(name) && VersionNameRegex().IsMatch(name);
    }
    ...
    public static string EntryPath(string packId, string version, string entry) {
        return Path.Combine(MountRoot(packId, version), entry.Replace('/', Path.DirectorySeparatorChar));
    }
    ...
    public static PackLocation? Resolve(string widgetPath) {
        ...
        string full;
        try {
            full = Path.GetFullPath(widgetPath);
        }
        catch {
            return null;
        }
        ...
        PackLocation? found = ResolveDirectory(dir);
        return found;
    }

    private static PackLocation? ResolveDirectory(string directory) {
        lock (ResolveCacheLock) {
            if (ResolveCache.TryGetValue(directory, out PackLocation? cached)) return cached;
        }

        PackLocation? result = null;
        string? current = directory;

        for (var depth = 0; depth < MaxMountDepth && current != null; depth++) {
    ...
    public static bool TryResolve(string widgetPath, out string packFile, out string entry,
        ...
        PackLocation? found = Resolve(widgetPath);
    ...
    public static string EntryPathIn(string mountRoot, string entry) {
        return Path.Combine(mountRoot, entry.Replace('/', Path.DirectorySeparatorChar));
    }

    public static string CacheDirFor(string packFile) {
        string key;
        try {
            key = Path.GetFullPath(packFile).ToLowerInvariant();
        }
        catch {
            key = packFile.ToLowerInvariant();
        }

        byte[] hash = SHA256.HashData(Encoding.UTF8.GetBytes(key));
        var folder =
            $"{Slug(Path.GetFileNameWithoutExtension(packFile))}-{Convert.ToHexString(hash)[..8].ToLowerInvariant()}";
    ...
    public static string MakePackId(string author, string group, string name) {
        ...
        IEnumerable<string> parts = new[] { Slug(author), Slug(NormalizeGroup(group)), n }
    ...
    public record PackLocation(
        string PackFileStr,
        string PackFolderStr,
        string MountRootStr,
        string PackIdStr,
        string VersionStr);
}