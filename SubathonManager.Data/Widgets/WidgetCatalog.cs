...
public static class WidgetCatalog {
    ...
    public static async Task<List<WidgetCatalogEntry>> RefreshAsync(
        ...
        List<WidgetCatalogEntry> rows = await db.WidgetCatalogEntries.ToListAsync(ct);
        var byPath = new Dictionary<string, WidgetCatalogEntry>(StringComparer.OrdinalIgnoreCase);
        foreach (WidgetCatalogEntry row in rows)
        ...
        foreach (string file in EnumeratePackages(root.Path)) {
            ...
            byPath.TryGetValue(stored, out WidgetCatalogEntry? entry);
            ...
            WidgetPackManifest? manifest = WidgetPackInstaller.ReadManifest(file);
        ...
        foreach (WidgetCatalogEntry row in rows.Where(row => !seen.Contains(row.PackPath))) {
    ...
    private static void Fill(WidgetCatalogEntry entry, string file, FileInfo info,
        ...
        entry.PreviewCachePath = ExtractPreview(file, manifest.PreviewImage, true) ?? string.Empty;
    ...
    public static async Task<WidgetCatalogEntry?> RefreshEntryAsync(IDbContextFactory<AppDbContext> factory,
        ...
        await using AppDbContext db = await factory.CreateDbContextAsync(ct);
        WidgetCatalogEntry? entry = await db.WidgetCatalogEntries.FirstOrDefaultAsync(e => e.PackPath == packPath, ct);
    ...
    private static string PreviewKey(string packFile) {
        ...
        return
            $"{WidgetPackPaths.Slug(Path.GetFileNameWithoutExtension(packFile))}-{Convert.ToHexString(hash)[..12].ToLowerInvariant()}";
    }
    ...
    public static void BuildIndex(IEnumerable<WidgetCatalogEntry> entries) {
        ...
        foreach (WidgetCatalogEntry entry in entries) {
            ...
            if (!byPackId.TryGetValue(entry.PackId, out List<WidgetCatalogEntry>? list))
    ...
    public static WidgetCatalogEntry? EntryForPackFile(string packFile) {
        ...
        lock (IndexLock) {
            if (_byPath.TryGetValue(stored, out WidgetCatalogEntry? byStored))
                return byStored;
    ...
    public static List<WidgetCatalogEntry> EntriesForPackId(string packId) {
        lock (IndexLock) {
            return _byPackId.TryGetValue(packId, out List<WidgetCatalogEntry>? list)
                ? [..list]
                : [];
        }
    }
